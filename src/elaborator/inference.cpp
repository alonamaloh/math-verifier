...
ExpressionPointer Elaborator::recoverClaimHint(
        ...
        // signposted at all.
        //
        // LAST resort, after every ordinary reading of the citation has
        // failed — mirroring the prover's own `reductio` placement. Sitting
        // any earlier is expensive: it re-runs the whole recovery chain
        // against `False` for citations that the Pi-goal rung or the
        // last-ditch diff would have closed. Measured at that earlier
        // position, `Algebra/rank_nullity` went 61.6 s → 86.2 s.
        //
