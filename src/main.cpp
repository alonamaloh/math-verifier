...
// `kernel rewrite --since-to-by FILE ...` — the C6 mechanical rewriter for
// the since→by synonym retirement. The lexer (not a text script) decides
// what is a `since` keyword: occurrences inside comments or identifiers are
// never touched. Token line/column are 1-based BYTE positions, so the byte
// span of each KeywordSince token is exact; every span is checked to spell
// "since" before any edit, and the file is rewritten only when all spans
// check out. The kernel re-verifies the result on the next build — that,
// not this tool, is the safety net for semantics.
int rewriteSinceToBy(const std::vector<std::string>& filenames) {
    ...
    for (const auto& filename : filenames) {
        ...
        for (const auto& token : tokens) {
            if (token.kind != TokenKind::KeywordSince) continue;
            ...
            if (offset + 5 > source.size()
                || source.compare(offset, 5, "since") != 0) {
                std::cerr << filename << ":" << token.line << ":"
                          << token.column
                          << ": internal error: expected 'since' at this "
        ...
        output << source;
        output.close();
        std::cout << filename << ": " << spans.size() << " since→by\n";
        totalRewritten += (int)spans.size();
    }
    std::cout << "total: " << totalRewritten << " since→by\n";
    return 0;
}
...
// The lemma-search ENGINE (computeGoalHits / computeMentionHits and
// helpers) lives in lemma_search.cpp, shared with the elaborator's
// failing-proof error messages. The CLI-specific cache loading,
// source-line lookup, and printing stay here.
...
axiom le_trans : (a b c : Nat) → LE(a, b) → LE(b, c) → LE(a, c)
    ...
    // Error-message surface (#1): a failing `done` whose goal matches a
    // library lemma's conclusion gets that lemma appended to the claim
    // error. Here `le_add_right` needs `LE(a, c)`, which is NOT in scope,
    // so the auto-prover genuinely fails — and the search suggestion
    // names the lemma the author should cite.
    {
        bool threw = false;
        std::string message;
        try {
            verifyMathSource(R"(
module Test.search_suggest
axiom Nat : Type(0)
axiom LE : Nat → Nat → Proposition
axiom plus : Nat → Nat → Nat
axiom le_add_right : (a b c : Nat) → LE(a, c) → LE(plus(a, b), plus(c, b))
theorem target (a c b : Nat) : LE(plus(a, b), plus(c, b)) := { done }
)");
        } catch (const ElaborateError& error) {
            threw = true;
            message = error.what();
        ...
        EXPECT_TRUE(threw);
        EXPECT_TRUE(message.find("search by conclusion shape")
                    != std::string::npos);
        EXPECT_TRUE(message.find("le_add_right") != std::string::npos);
    }
}
...
static int kernelMain(int argc, char* argv[]) {
    ...
    if (argc >= 3 && std::string(argv[1]) == "rewrite") {
        // kernel rewrite --since-to-by FILE.math [FILE.math ...]
        ...
        if (mode == "--since-to-by") {
            ...
            return rewriteSinceToBy(filenames);
        }
        std::cerr << "rewrite: unknown mode " << mode
                  << " (expected --since-to-by)\n";
        return 1;
    }
    ...
    if (argc >= 2 && (std::string(argv[1]) == "-h"
        ...
        std::cout << "Usage:\n"
                  ...
                  << "                      --check-redundant-by / -by-non-eq /\n"
                  << "                      -calc-steps: warn at each `by` (on `=`\n"
                  << "                      steps / on non-`=` steps / on whole calc\n"
                  << "                      intermediate steps) the auto-prover could\n"
                  ...
                  << "\n"
                  << "Environment:\n"
                  << "  MATH_CHECK_REDUNDANT=1   turn on all three redundancy\n"
                  << "                           diagnostics for a whole build\n"
                  << "                           (e.g. `MATH_CHECK_REDUNDANT=1 make\n"
                  << "                           -B library`), instead of the\n"
                  << "                           per-file --check-redundant-* flags.\n"
                  << "  MATH_REDUNDANT_BUDGET=N  kernel-step budget for a redundancy\n"
                  << "                           check's by-less re-proof (default\n"
                  << "                           10000): a `by` is reported redundant\n"
                  << "                           only when it re-proves under N steps,\n"
                  << "                           so cleanup never trades a fast proof\n"
