...
| 2 | B1–B3 tier skeleton, context index, cast tier | **B1/B2/B3 done; residue forensics done — verdict: no norm_cast mechanism needed; F1–F5 queue CLOSED 2026-07-11** | tier-4 sign index v1 (21b1cb4) + v1.1 IsNonneg/alternatives/form-bridges: closes-today 36.2%→42.3% (+352 sites). B3 cast retry landed (test-proven; yield compounds with future families). Per-key vectors landed. **B3-proper audit (2026-07-02 night): the tier3-cast bucket (445) is DIFFUSE — 170 sites are hand-built `<term>` proofs, top named lemma 14 sites, shapes mostly cast-bearing EQUALITIES (Integer to_integer/absolute_value chains), not order judgments. No quick prototype exists; it needs the full morphism-packet + norm_cast design. DECIDED (delegated, 2026-07-02): convention-named lemma discovery — no new syntax; missing-slot warnings make it auditable; a packet clause only if discovery proves too magic.** **B1 stage-1a memo tried and REVERTED (73ae506): A/B showed zero effect — the 2026-06-27 perf session already satisfied tier-0's speed rationale (derivative ~4.9s, continuity ~1.3s, full re-verify 25.5s wall today). B1's remaining value = the A2 statement-address spine + derived-fact blackboard, not speed.** **A2 stage-2 verdict on that remaining value (2026-07-11): the spine is NOT built — `given()`'s linear defeq scan serves ~16 call sites, the prover's fact collection is already memoized, and the blackboard has neither a measured perf nor a readability motivation; build spine+blackboard together if a re-derivation cost is ever measured (see the A2 stage-2 design block).** **Zeroness DECIDED by measurement (2026-07-02 night): post-breadcrumb-sweep, the classifier shows the `= 0` slice at single-digit sites (the sweeps consumed it) — no dedicated judgment family; the equality battery suffices. Post-sweep bucket sizes (3868 hinted sites, down from 5807): tier3-cast 465, B4-order-step 308, tier2-ground 268, tier4-sign 189 — B4 is now the largest well-specified target.** A2 spine: stage 1 landed (see row 5). **B3 scoping sharpened (2026-07-03, classifier re-run at the post-flip tree: 3872 hinted sites, closes-today 981/25.3%, tier3-cast 412, tier3+4-sign-cast 74, tier2-ground 242, B4-order-step 98):** the tier3-cast bucket's shapes are already LEAF-cast (`to_integer x * to_integer y = |z| * to_integer w` — atoms under single casts), so castPushToLeaves (which EXISTS, proof-carrying, gated on the `<hop>.<op>_preserves` convention — cast_normal.cpp) is NOT the missing piece for them; the yield is in cast REFLECTION/lowering (`ι a = ι b ⟸ a = b`, order twins) + CONTEXT-FACT cast normalization so goal and hypotheses meet at one placement. Staged design for the fresh-context build: B3.1 equality tier (push both sides + injective lowering when both sides are same-hop casts, then battery retry on the lowered goal); B3.2 order transport (normalize/lower endpoints, feed B4); B3.3 context-fact normalization; B3.4 packet audit warnings at coercion registration (slots: zero/one/add/multiply/subtract/LessOrEqual/LessThan preserves + reflects/injective; naming today is INCONSISTENT — Rational.to_real.LessOrEqual_preserves vs Real.from_natural_add_preserves — the owner wants uniform names, see embedding_order_lemmas_uniform_naming memory; adopt `<hop>.<op>_preserves`/`<hop>.<op>_reflects` and sweep). Measure each stage against the classifier baseline above (log at scratchpad classify_before.log, regenerate with MATH_CLASSIFY_HINTS=1 make library + scripts/hint_classification_report.py). **B3.1 LANDED AND MEASURED (2026-07-04): the cast-equality tier** (tryCastEqualityTier, cast_normal.cpp, hooked at the equality battery's decline point in autoProveCalcStepRaw) — leaf-normalize both endpoints via castPushToLeaves; when both become the SAME single-hop cast `ι(a)`/`ι(b)`, lower to `a = b` at the source carrier, re-run the battery there, lift by ι-congruence; else retry once at the normalized endpoints (move-guarded, terminates). Cost: +0.8s wall on the full rebuild (~2%, A/B-measured). **Yield: +3 closes-today (1045→1048 of 3937) — goal-side lowering ALONE is nearly dry, confirming the audit's prediction: the tier3-cast bucket (411, 164 of them hand-built `<term>` proofs) needs B3.3 CONTEXT-FACT normalization so hypotheses and the lowered goal meet at one placement.** Fresh-session plan stands: B3.3 next (highest yield), then B3.2 order transport (feeds B4), then B3.4 packet-audit warnings + the uniform `<hop>.<op>_preserves`/`_reflects` naming sweep. Baselines: classify_before.log / classify_after_b31.log in the session scratchpad; regenerate per the recipe above. **THE B3 BLOCK IS DONE (2026-07-04 second session; 3b22980b, abd0aaea, 33d4507f, ce316f54). Forensics first: cast-tier decline telemetry (MATH_CAST_TIER_DEBUG) over a classifier run re-cut the tier3-cast bucket into three implementable families and exposed two measurement artifacts — (i) the classifier's `closes` bit is the 1000-step redundancy budget, so cast-tier wins (defeq/type-inference heavy, context-sensitive) are systematically UNDER-COUNTED: a site that declines in the classifier closes at full budget in isolation (probe-proven); (ii) ~15 'expensive by-less' warnings pre-exist at HEAD on any full re-verify (warm builds mask them) — warning GATES must diff site lists, not count to zero. B3.3 (3b22980b): castPullToRoot (factoring dual of push-to-leaves — endpoints whose every leaf is an ι-image or zero/one-constant re-express as ι(source)) gives the equality tier COMPOUND lowering (ι q·ι m + ι r = ι j drops to q·m+r = j where the facts live); tryCastNormalizedHypothesisMatch in tryClassifyDiff's decline path matches equality hypotheses at leaf-cast placement + lowers ι x = ι y facts through <hop>.injective; `divide` joined the binary-descent suffixes (descent only — no divide_preserves exists). B3.2 (abd0aaea): tryCastOrderTier claim tactic — pull-lower through <hop>.<Rel>_preserves with a BOUNDED lowered prover (hypothesis scans + mono/sign recursions + next hop; the full claim battery on speculative lowered goals blew Real/division's budget — a sign-split probe `(2:Real)<0` recursed three carriers deep), push-retry at leaf spelling, reflects-lowered facts (STATED-spelling parse first — WHNF unfolds Integer.LessOrEqual to IsNonneg past recognition). Also landed: tryDisjunctiveSyllogism (A∨B + ¬A ⊢ B, conjunction legs participate, cheap-tier) — already simplified a library proof at merge. B3.4: make audit-coercions + MATH_AUDIT_COERCION_PACKETS (33d4507f) reports missing packet slots per hop; the five missing lemmas + re-homing + the from_natural.* uniform renames landed via Sonnet sub-agent (ce316f54, diff reviewed line-by-line) — to_rational and to_real packets now COMPLETE, to_integer misses only structural ℕ slots (subtract/negate). Wall-clock cost of the whole block: ~+1s on a 34s full re-verify. Classifier closes-today 1040→1051 (capped instrument; feature tests cast_context_test/cast_order_test pin the real by-less capability at full budget). Residuals: mechanism C v1 matches structurally only; equality-side hypothesis matching does no defeq fallback. **BUDGET DECISION + C6 SWEEP (2026-07-04 fifth session): the standing lint budget STAYS at 1000** (its contract — flagged = free deletion — is the right developer-facing invariant; the round-trip argument in redundancyBudgetValue's comment holds). The sweep instead ran the checker at `MATH_REDUNDANT_BUDGET=10000` (5× under the 50k expensive-step warning threshold, ~1ms/site worst case, so nothing deleted under the sweep budget can approach warning territory); flags at that budget are advisory-for-sweeps, not the standing lint. **THE SWEEP IS DONE (4d1f671d): 550 flags / ~90 manifest files → ~410 plumbing hints deleted, ~140 substantive keeps (IH, split/partition equations, bisection case-defining lemmas, headline citations), ~55 name cascades settled; zero performance-restores needed (the 10k bound held — the only post-sweep warnings were 3 incomplete-cascade dead names). Agent-error taxonomy for future sweeps: (α) a cascade must NEVER drop `as` from a flat `suppose` (grammar requires it — no anonymous form; parse error at the deps scan poisons library-depends.mk, rm it when recovering); (β) agents can delete the un-flagged hint ADJACENT to a flagged keep-site (factorial: the IH line was flagged, the load-bearing neighbor got deleted) — the gate build catches it, match failures against the flag list before blaming the checker; (γ) proof-only edits do NOT cascade downstream rebuilds (iface-content cut), so post-sweep warning diffs only cover re-verified files — hidden baseline sites in untouched modules stay valid by construction.** **B3-RESIDUE FORENSICS + norm_cast DESIGN VERDICT (2026-07-11): the tier3-cast bucket is NOT a norm_cast gap — no dedicated cast mechanism is warranted.** Fresh baseline (post-ground-re-export tree, full re-verify with `MATH_CLASSIFY_HINTS=1`): 3755 hinted sites (since: 0 — retired), closes-today 1158/30.8%, tier3-cast **443** (the re-export consumed tier2-ground 242→155, not this bucket). A per-module probe of all 71 bucket modules with `MATH_REDUNDANT_BUDGET=10000 --check-redundant-by` splits it: **183/443 (41%) close at 10k budget** — the landed B3 machinery covers them and the classifier's 1000-step `closes` cap hides it (of these, the ~110 manifest-file flags are the C6 sweep's deliberate keeps; the rest live in not-yet-cleaned files). The 260 true decliners, classified by reading the actual `by`-text at every site: **~157 content citations** correctly on the page (88 named-lemma incl. helper dispatch + bezout recursion; 55 local-fact instantiations fClose/tailClose-style; 14 sign_split case-splits) — bucketed cast ONLY because ε/4 and ι-numerals print as tower casts (`Rational.to_real (Integer.to_rational (Natural.to_integer 4))`); **25 ∣-through-cast** (IntegerMod/GaussianInteger divides_add/negate/multiply_right/of_natural_divides chains); **59 bridge forms** (23 `by substituting (…)`, 16 `by (<prop>)`, 20 inline sub-chains — mixed removable/content, see blind spot below); **12 field/ring steps** (`ε/4 + ε/4 = ε/2 by field`); 4 injectivity-transports + 3 unfolding. **Probes proved:** (π1) `ι(K+1) = ι(1+K)` under the two-hop compound cast closes BARE as a calc step (archimedean:289's exact shape) — the B3.3 pull-lowering works; the hint survives only because of (π2); (π2) **the redundancy checker NEVER probes `by (<proposition>)` or `by substituting (…)` hinted steps** — a reproduction whose bare twin verifies stays unflagged — so the 41% artifact share is an UNDERCOUNT and every sweep to date has been blind to these forms; (π3) `abs(x − 0) = abs(x)` closes bare (the limits/derivative subtract-zero bridges are removable, checker-invisible per π2); (π4) `ε/4 + ε/4 = ε/2` genuinely declines — **the by-less battery has no `field`** (ring only); that family is cast-incidental. Packet audit state: to_rational + to_real complete, to_integer missing only structural ℕ slots (monus — correctly absent), **GaussianInteger.embedInteger fully packet-less** (the lemmas exist under ad-hoc names: coordinates_injective, embed_zero, …), and **the ℝ↪ℂ hop is INVISIBLE to `make audit-coercions`** (coercion_packet_audit.math never imports ComplexNumber.embedding). **PROPOSED NEXT (ranked, no elaborator code yet): (F1) checker fix** — probe `by (<prop>)`/`by substituting (…)` steps in --check-redundant-by (cheap, makes all measurement honest, unlocks the removable-bridge harvest); **(F2) by-less `field`** — cost-gated field attempt in the equality battery when an endpoint carries `divide`/reciprocal at a field carrier, nonzero side-goals via the existing structural fallback (absorbs the ε/2 family; the dont_justify_routine_computation enabler); **(F3) ∣ joins the transportable relations** — new packet slots `<hop>.divides_preserves`/`_reflects` (re-home Integer.divides_of_natural_divides / natural_divides_of_integer_divides under uniform names), tryCastOrderTier transports ∣ endpoints like ≤/< (~25 sites); **(F4) ≠-transport** — `a ≠ b ⊢ ι a ≠ ι b` via the existing injective slot in the cast tier's Not(=) path (~6 hand-built suppose-blocks incl. to_rational.LessThan_preserves); **(F5) packet-audit sweep** — add ComplexNumber.embedding to the audit module + build the ℝ↪ℂ and embedInteger packets from the existing lemmas (renames/aliases), teach the audit that order slots are N/A for unordered carriers so `complete` is reachable. DEFERRED: diff-leaf hypothesis+evaluation composition (absolute_value_multiplicative's inline chains — they read fine, readability_is_primary); classifier cast-bit tightening (exclude pure-numeral tower casts so the bucket measures reasoning-relevant casts — do after F1/F2 change the baseline anyway). The standing decision holds: convention-named discovery, no new syntax, no packet clause.** **F1 + F2 LANDED same day (d668c355, 021991fa, a25fe436). F1 (checker fidelity):** fact-citation calc hints probed (the `!fromFactCitation` exclusion dated to the original `by (<fact>)` commit with no rationale); calc equality probes — checker AND classifier `closes` bit — run the full `autoProveClaim` instead of `autoProveCalcStep` (faithful to the bare-step one-path unification; the expensive-guard now measures TRUE bare cost, so 15 over-promising flags retracted); `by substituting` claims get classifier + redundancy diagnostics via the shared `reportClaimHintDiagnostics` helper (they were invisible to BOTH instruments — the classifier's hinted-site total jumped 3755→4439 just from seeing them). Harvest on the 71 tier3-cast modules at the 10k sweep budget: 973→1262 flagged sites (+304). **F2 (by-less `field`):** `tryAcRearrangement`'s phase-2 fallback tries `field` after `ring`, gated on an endpoint mentioning the carrier's divide/reciprocal and reentrancy-guarded (`inSpeculativeFieldAttempt_` — the nonzero side-condition discharge runs the claim prover). `ε/4 + ε/4 = ε/2` and `(1/a)·(1/b) = 1/(a·b)` close bare at ℝ and ℚ (Test/field_battery_test pins claim + calc-step + ℚ + context-hypothesis shapes); full clean re-verify 20.8s wall with an EMPTY warning-site diff; classifier closes-today 1487→1498 (capped instrument — the in-situ ε/2 steps exceed the 1000-step probe budget, so the standing lint keeps calling their hints speed-load-bearing; at the 10k sweep budget they flag, and a first-pass sweep deleted 9 field/ring hints across Real/division, Rational/division, cauchy_complete). **F3 + F4 LANDED same day (cd15f6d4). F4:** tryCastNotEqualTier (cast_normal.cpp, after the cast-order tier in both prover lists) — `¬(ι a = ι b)` closes bare from `¬(a = b)` through `<hop>.injective` (iterated, swapped facts via symmetry) and the reflects twin through ι-congruence; built terms verified against the goal as stated. The lint immediately flagged archimedean's two hand-built ≠-transport suppose-blocks as vestigial — deleted (Test/cast_not_equal_test pins 1/2-hop + swapped + reflects). **F3 needed NO elaborator code:** the ∣ bridges existed but weren't `automatic` — the library scan refuses non-automatic lemmas for bare closes (the error message even says so). Re-homed as `Natural.to_integer.divides_preserves`/`_reflects` + automatic, 9 citation sites renamed; audit checks ∣ slots where BOTH carriers have a divides relation (field targets can't state the lemma); Test/cast_divides_test pins both directions. Gates: full re-verify 21.0s, warning-site diff vs pre-F3/F4 EMPTY, error-tests 51/0, audit shows to_integer missing only structural subtract/negate. **F5 + HARVEST + DIVIDES PASS LANDED (2026-07-11: bcf93989, c134b3bd, e68800cc) — the F-queue is CLOSED. F5:** GaussianInteger.embedInteger and ComplexNumber.from_real packets built and `complete` — ad-hoc lemmas re-homed under the uniform slot names (embed_zero/embedInteger_one/embedInteger_multiply; from_real_zero/_one/_add/_multiply/_injective), six NEW lemmas written (ℤ[i]: add/negate/subtract_preserves + injective via the coordinates route; ℂ: negate/subtract_preserves via the cancel-then-`ring` shape — `ring` works at both RingModulo carriers), audit module imports the packet homes (the ℝ↪ℂ hop was invisible), and the audit now checks ≤/< slots (like ∣) only where BOTH carriers have the relation — unordered targets (ℤ[i], ℂ) skip them structurally, so `complete` is reachable; to_integer correctly still reports structural subtract/negate. Export-check 2554 decls. **Harvest sweep:** fresh 10k-budget scan over the 160-file clean manifest = 460 flags / 82 modules, zero checker aborts (the row-8 β bug never bit). 8 parallel Sonnet judgment batches + 1 cascade batch: ~250 routine hints deleted, ~160 deliberate keeps (IH / case-defining split equations / citation-IS-the-idea / symmetric twins — re-flagged C6 keeps fell only with a reason), 74-warning unused-name cascade settled + 4 second-order releases; net −175 lines; NO new expensive-by-less sites (only the pre-existing Real.series:77). **Divides pass:** exactly 3 of the 9 renamed to_integer.divides_* citations flagged (all IntegerMod/field) and fell with 6 more routine hints there; the other 6 declined even at 10k — load-bearing, kept. **Bonus landings:** (1) anonymous `suppose P;` now parses — the unused-name warning prescribed dropping `as h` but the grammar hard-required it (row-8 α is obsolete; take-header/modifier forms already had the anonymity machinery; reference.md updated); (2) the expensive-by-less warning now NAMES its winner for contextEqualityBridge (rewrote with `X` — try `by substituting X`) and transitivityBridge (chained facts list), via eq.source/fact.source threaded into lastWinningDetail_ — all 7 standing warnings are now actionable paste-ins. Gates across all three commits: make check fully green, error-tests 51/0, warning-site diff vs pre-F5 HEAD EMPTY. Row 2 has no open work; next-in-plan lives in rows 4 (B4 residue), 5 (A2 stage 2), and the D residuals.** |
| 3 | A1 keyword-free claims/calc | **DONE — keywords retired** | **Residual CLOSED 2026-07-10:** the 13 stale kernel self-tests (11 asserting retired `calc`/`claim`/`obtain`/`by_induction` syntax — already covered by `ErrorTest/*_retired` — plus 2 pretty-printer tests predating numeral rendering) are deleted/updated; `make check` is green again (it had been red since the retirements; the same run also found the un-re-armed clean-leak ratchet from e433b96f). Stage 1 (2026-07-03, 3b3126e): bare `P;` / `P by V;` / `P as NAME;` at statement position = a claim (desugars onto the claim wrapper — shared elaboration/unused-tracking/errors); bare proof terms state their type; blocks may end by restating the goal (the final-expression `E}` / `E;}` shapes keep their exact old meaning, so data-valued blocks are untouched). Activation bug found+dodged: the claim branch's dead done/okay wrapping path mis-binds de Bruijn when a `done by X` before `}` becomes a binding (goal-sentinel proposition passes the non-null check) — done/okay stay excluded from the statement loop; that latent path is still dead and should be deleted or fixed when the migration touches it. PARITY LANDED (post-26bc0be): the bare-statement by-tail routes through parseStructuredClaimTail — by-cases/by-substituting/by-induction/unfolding all work on bare statements; the MIGRATION SWEEP is now unblocked. **PILOT DONE (535d5e23): library/Integer keyword-free — 100 claim sites across 14 files, 3 subagents + batch build; found+fixed a real chain-probe bug (bare `k = 0 ∨ …` mis-parsed as a chain — detection now requires a full-chain speculative parse with ≥2 steps ending at a statement boundary) and the truncated-library-depends.mk-after-parse-error gotcha. LEAVE-LIST TAXONOMY for the full sweep: terminal `claim by cases {…}` (no proposition — keep or redesign per A4); semicolon-less claims in EXPRESSION position (`:= claim P by X claim by cases {…}` bodies — spec decision needed, likely braces+statements); claim-last-before-} (bare `P;}` means final-expression — keep claim or end with done). **LIBRARY-WIDE SWEEP DONE (owner-approved 2026-07-03, commits e22f001a..0e841e42+tail): ~4400 claim/calc statement sites converted across Natural, Rational, Real, ComplexNumber, IntegerMod, Algebra, Lists, Set, Polynomial, GaussianInteger, Integer, Logic, RingModulo — 6 waves of 4-5 subagents (Sonnet fine for these, owner request: use Sonnet for mechanical batches), full clean validation per wave, zero-to-two fixups per wave. REMAINING keyword population (the keep-list, pending A4/A5 and the expression-position story): terminal `claim by cases {…}` heads; claims/calcs in EXPRESSION position (`:= calc` bodies, `↦`/`=>` arm bodies, parenthesized args, `witness … with (calc …)`, semicolon-less `:= claim P by X claim by cases` chains); claim-last-before-`}` (bare `P;}` = final-expression); Leave-rule (f) RETIRED (25b59f65): equality claims route lambda hints through tryUnderBinderStep exactly like calc = steps — the two forced calc keywords went bare. KEYWORD REMOVAL from the parser stays gated on: A4 (case-analysis redesign kills terminal `claim by cases`), the expression-position chain story, and rule-f. Owner also asked for a CIC-leak sweep (pattern matching, Or.introduceLeft, …) — planned as the post-A4 follow-up sweep.** Connectives note (ba0943d, since REMOVED entirely by owner call 9bc0c46): `then`/`hence`/`therefore` parse as noise words before bare statements (contextual, operand-start lookahead guards the if-then-else sugar; no recency-hint plumbing needed — the fact scan is already most-recent-first). Chains LANDED (127e890): a relation chain at statement position is a calc with no anchor word — rewound additive-level speculation detects the shape, the existing calc statement branch handles it via parseCalc(consumeCalcKeyword=false); single relations keep the claim route; mixed chains + per-step by + as-naming + connectives compose. REFERENCE-TARGET GAP LIST (checked against the idealized form 2026-07-03): bare-by statements ✓, from-form ✓, connectives ✓, chains ✓, choose-without-from ✓; still missing (a) STATEMENT-FORM structural steps (A3/A4/A5 territory; the induction HEADER is settled — owner-designed `by_strong_induction on m with hypothesis no_smaller_solution` LANDED (contextual word marks the IH, subject defaults to shadowing the inducted variable; verbose `with <subject>, <ih>` stays for non-variable `on` targets) — what remains is the braces-to-flat-statement transformation), (b) whole-body `by lemma` plumbing — Or.self collapse LANDED (0944d67) + the literal `:= by <lemma>` body spelling LANDED (4e70823: after `:=` a by-headed body desugars to the goal-restating claim) — **two_divides_root now verifies in its EXACT idealized spelling**; REMAINING: goal-side intro + ∃-flattening for the ¬∃ shape (sqrt_two_irrational := by no_double_square), (c) the acceptance form's `contradiction with <fact>` must be rewritten to the owner's done-idiom (terminal skipped by DECIDE), (d) prover strength on the bare `hence n*n = 2*(k*k)` ring-cancellation step. **Idealized no_double_square PROBED end-to-end (2026-07-03, scratchpad ideal_probe.math): parses and elaborates through the chain/from/hence forms; first failure is the FIRST contradiction block's `done`** — the chain binds `subject = 0` but False needs the transported absurdity. Probe-driven fixes for next session: (i) end reductio chains AT the absurdity (`then 1 ≤ subject = 2 * 0 = 0;` binds `1 ≤ 0`, and the Not-peeled pool + premise discharge should close False via not_less_or_equal_successor_zero — verify); (ii) mark `Natural.lt_irreflexive` automatic (the second block binds `s*s < s*s`; Real's twin is automatic, Natural's is not — [[reductio_done_idiom]] alignment, cheap); (iii) the second block's chain steps (s*s ≤ n*n from s ≤ n via mono index; n*n < 2*(n*n) needs 1 ≤ n*n) may need by-hints — measure which close bare; (iv) ∃-FLATTENING DESIGN (the remaining whole-body-by piece, for sqrt_two_irrational := by no_double_square): in the by-hint path only, depth-bounded — goal WHNF-Pi ⇒ λ-intro + recurse; else eliminate a context Exists via verbose `Exists.eliminate.{u}(A, P, G, λ x hp. body, h)` and recurse with x, hp in scope; CLOSED-in/CLOSED-out convention throughout (internal.hpp ~476), delicate de Bruijn — fresh context required. **Probe iteration 2 (2026-07-03, same session): fixes (i)+(ii) LANDED AND VERIFIED** — chains-to-the-absurdity close the first reductio (`then 1 ≤ subject = 2 * 0 = 0; done` works via the Not-peeled pool), and Natural.lt_irreflexive is automatic (8fadcd3). The probe now verifies THROUGH both reductio blocks and the descended equation; the two remaining stoppers, precisely: (α) the single-position diff walker in the claim-by/coercion transport recurses PAST the rewrite point when both sides share a head (fact `s*s ≤ n*n` vs target `2*(n*n) ≤ n*n`: multiply-vs-multiply descends to a two-position diff and declines, though `equation : s*s = 2*(n*n)` rewrites the whole operand in one step) — fix: also try each whole mismatching subtree as ONE rewrite leaf against in-scope equalities before recursing; interim spelling is `claim … by substituting equation`; also from-with-STATED-PROPOSITION facts skip transport entirely (bridgeCitedFact path) — same fix location; (β) `¬(a ≤ b) → b < a` does NOT exist at Natural (the old proof case-split on compare_strict instead) — add `Natural.lt_of_not_le` to order.math (via trichotomy), then the final `done by no_smaller_solution` premise-discharge chain can derive `n < subject` from the second reductio's fact. Probe file: scratchpad ideal_probe.math (copy into Test/ when it goes green as the acceptance test). **GREEN (2026-07-03, 2475868): Test/no_double_square_idealized.math LANDS as the acceptance test** — both stoppers closed same session (α FIXED (post-2475868): findUniqueDiffPair is a bottom-up coarsening walk — a node whose children carry multiple distinct diffs is itself the diff; whole-subtree rewrites transport (probe green); previously-found pairs unchanged; β: Natural.lt_of_not_le moved to its honest home in Natural/order.math via trichotomy — it existed BURIED in Polynomial/bezout.math, duplicate deleted, bezout's prover-found use now hits the canonical one). Deviations from the plan text, each owner-sanctioned or queued: done-idiom reductio endings (owner DECIDE), by_strong_induction braces (statement-form steps pending, A3-A5), the substituting step (α), and two derivation steps carry `by` hints the idealized text leaves bare (lt_of_not_le, successor_less_or_equal_multiply — prover-strength residual). Lux-gate remainder: sqrt_two_irrational := by no_double_square needs the ∃-flattening; then the gate question goes to the owner. **∃-flattening step 2 LANDED (7c1cf78): citePiGoalByIntroduction WHNF-peels (¬ goals intro their supposition) + citeCoreGoalWithExistsFlattening (direct attempt, else eliminate an introduced Exists binder into witnesses via verbose Exists.eliminate, recurse; depth-bounded, by-hint only). Feature-tested GREEN: single ∃, ∃+∧, nested ∃. LAST GAP (rung 3): nested ∃ WITH a ∧ under the inner one — the premise back-inference (completeCitationFromBindings pass (a), induction.cpp ~1514) meets the whole conjunction as ONE eliminated fact `h1 : (λn. And(1≤m', And(1≤n, eq)))(w1)` and cannot pin both witnesses: it scans BINDER TYPES only (no ∧-leg decomposition there) and its first-match commit has no backtracking. Fix directions to evaluate with instrumentation, fresh session: (i) decompose conjunction legs of context facts in pass (a) (mirror collectLocalBinderFacts), and/or (ii) unique-match-first premise ordering (collect all candidate facts per premise; commit only unique matches each round; ambiguous ones only when no progress). Suspicious detail worth checking first: WHY rung 2 (single ∃+∧, same conjunction-as-one-fact shape) PASSES — find which pass discharges its legs before assuming (i) is needed.** **RESOLVED SAME SESSION (33a9ca8): And-typed binders now DECOMPOSE in the flattening scan (two lambda binders applied to the And.left/And.right projections, alternating with the Exists eliminations) — rung 3 AND the ∃-inside-∧ variant both close, and **Natural.sqrt_two_irrational IS NOW THE IDEALIZED ONE-LINER `by Natural.no_double_square`**. ALL THREE reference-target theorems verify with their whole-proof citations. THE LUX GATE QUESTION IS NOW THE OWNER'S: the remaining textual deltas between the acceptance test and the plan's idealized form are (1) statement-form by_strong_induction (A3-A5, braces remain), (2) the owner-sanctioned done-idiom reductio endings, (3) one `by substituting equation` step (diff-walker fix queued), (4) two prover-strength `by` hints. Whether that clears the 'idealized form' bar — and triggers the Lux rename — is an owner call. **OWNER WEIGHED IN (2026-07-03): the library file itself must carry the ideal form — the cases-on-compare_strict pattern match and the positional inductionHypothesis(...) call were the remaining anti-patterns. DONE (3ee9222): Natural/sqrt_two_irrational.math now holds the idealized no_double_square (reductio + `hence n < subject by Natural.lt_of_not_le` + argument-free `done by no_smaller_solution`); Natural.compare import dropped; the Test duplicate deleted (the library file IS the acceptance form).** Remaining textual deltas from the plan's exact reference text (updated after 2d0260c): statement-form by_strong_induction (A3-A5), done-idiom endings (owner-sanctioned), the two-hop squaring chain (`s*s ≤ s*n = n*s ≤ n*n` — the one-step `s*s ≤ n*n` needs two-operand mono composition in B4), and two substantive `by` hints (lt_of_not_le, lt_multiply_of_two_le). GONE since the owner's successor/claim feedback: the successor spellings (lt_multiply_of_two_le, `<`-stated + automatic, ABSORBED successor_less_or_equal_multiply; factorization + prime_factorization_unique restated in `<`), the lone `by substituting` step and the 1 ≤ 2 / 1 ≤ n*n seeds (the single reductio chain ends at the absurdity and everything else discharges bare). ALSO fixed at source: writeCacheFile is atomic now (temp+rename — an interrupted parallel build left truncated .mathv files that poisoned incremental builds with 'short read').** **Owner nitpick round 2 (2026-07-03, 38aea21): the A7 `from`-form is REMOVED by owner verdict (plain `P by fact` already transports — distinction without a difference); `then`/`hence`/`therefore` are REMOVED ENTIRELY (9bc0c46 — owner call: the parser tolerance too; the A1 connectives design bullet is dead); breadcrumbs left the page via two additive prover changes (two-pass premise back-inference: historical greedy first, unique-match-first on failure; search-free ground-premise lookup under the scan against premise-free automatic facts — Natural.one_le_two is the first, the stop-gap pattern for [[arithmetic_decision_tactic_idea]]); two_divides_root renamed two_divides_of_two_divides_square. STILL OWED from the nitpick list: (a) k² notation LANDED (27fbfbb: postfix ² token, parse-time desugar to `E * E` — downstream systems see plain multiplication; sqrt file restated; printing-side superscript compression = existing printer work item); (b) `done by no_smaller_solution(n, k)` — positional-data-args mapping LANDED in recoverClaimHint (data args fill non-Prop slots in order, premises become holes) but BLOCKED on a deeper gap: explicit-hole citations of LOCAL-BINDER facts don't premise-discharge (probe: scratchpad positional_probe.math, `by L(a, ?, c, ?)` fails with premises stated in context) — RESOLVED (26bc0be): the hole dispatch only knew GLOBAL heads; a local-binder branch routes to the same solver, and the positional form works end-to-end — the sqrt descent closes with `done by no_smaller_solution(n, k)`; **∧-leg premise discharge LANDED meanwhile (e563c45): the back-inference's hypothesis scan decomposes conjunction binders into legs (projection proof terms, worklist) in the unique-match-first retry pass only — additive; grouped hypotheses (bothPositive-style) and unpacked-existential bodies now discharge citation premises without restating legs;** (c) the mathematician-invisible rearrangement `2*(n*n) = (2k)(2k) = 2*(2k²)` still needs one chain on the page — absorbing it = ring-normalize-then-cancel prover composition (the cancellation tier).** THEN the migration sweep, which ANSWERS the keep-vs-remove DECIDE. **TERMINAL-CLAIM SWEEP DONE (2026-07-04): every proposition-less `claim by <hint>` (188 sites / 75 files — `claim by cases` 164, `by substitution/substituting/induction/<lemma>` the rest) is now `done by <hint>`, via `kernel rewrite --terminal-claim-to-done` (lexer-driven: a `claim` token IMMEDIATELY followed by `by` carries no proposition and is exactly `done`); zero fallout, full validation green. The A4-gated keep-list item is CLEARED — the terminal form has its keyword-free spelling. THE REMAINING claim/calc KEYWORD POPULATION (counted 2026-07-04): ~258 `claim <proposition>` sites (expression-position: parenthesized args, `:=` chain bodies, semicolon-less chains — plus stragglers to audit) and 41 expression-position calcs (`(calc` 32, `:= calc` 5, `=> calc` 2, `↦ calc` 2) + statement calcs that keep the keyword legally. KEYWORD REMOVAL therefore still gates on the expression-position design (braces+statements spec) — the last gate standing; docs reteach `done by cases` as the split spelling.** **THE EXPRESSION-POSITION GATE IS CLEARED (2026-07-04 third session, afd35766 + 691df8b2 + e27d4323 + 5bf78bb6 + 8b8578b3 + da48b2be): the A1 endgame engine + the full keyword sweep are LANDED.** Engine (Opus agents against written specs, all diffs reviewed): (1) dual-reading block tail — SurfaceBlockTail, resolved at elaboration (Prop goal: direct reading first, accepted only if it PROVES the goal via bridgedResultProvesGoal; then a prove-once+coerceToExpectedTypeViaDiff reading; then the statement+auto-close fallback; data blocks byte-identical); (2) statement-shaped `:=` bodies (chains + P-by claims) and chain-only `=>`/`↦`/`|`/`case:`/witness-with/if-branch/by_representatives bodies (a trailing by/as in arm position belongs to the enclosing statement — claim tails are `:=`-only BY DESIGN); (3) body chains wrap as BlockTail(calc) so the final-calc DIRECT path survives (with AutoProverBudgetError caught in speculative readings — an escape failed the build once); (4) the dead done/okay wrapping path (latent de Bruijn mis-bind) deleted; (5) claim/calc citation PARITY: recoverClaimHint gained the calc-step mismatch pipeline — tryApplyBareLemmaToDiff (THE mechanism: solve an unapplied lemma against the goal equality's differing subterm, congruence-wrap), desugarRewrite fallback, tryDiffWrapForEqualityGoal; (6) prover quotientSoundBridge (forward twin of the exact bridge: class-equality goals close from rep cross-equation facts). Sweep (17 Sonnet agents over 5 tranches, ~1000 sites): all of library/ + Test/ keyword-free EXCEPT four grammar-pinning tests (claim_as_statement_test, goal_by_test, structured_proof_test, calc_statement_test — Phase 3 repurposes them); the `P by calc` population (132 sites) restyled per no_claim_by_calc (composite==P → bare chain statement; else `P by { chain }`); absurd(calc) sites moved to the done-idiom (constructor-disjointness closes the False claim; NON-False arm goals need an explicit `False;` before `done`). **A1 ENDGAME CLOSED (2026-07-04 fourth session: 9277c94e, 761563b9, b6ed6d4b, 83dc2c77). (a) PHASE 3 DONE (b6ed6d4b): `claim`/`calc` are migration ParseErrors at all four consumption sites (tokens stay reserved, per obtain); dead branches deleted (the legacy speculative claim statement branch, looksLikeLegacyClaim, isStructuredClaimTerminator, the isClaim plumbing in the let-wrapper chain — parseStructuredClaim now parses exactly the done/okay closers); claim_as_statement_test folded into bare_statement_test (4 cases) and deleted; calc_statement/structured_proof/goal_by repurposed keyword-free; ErrorTests claim_retired/calc_retired added + 11 existing bodies rewritten so their intended errors still fire (subtlety: a bare 1-step relation with `by` routes through the plain citation elaborator, NOT calc.cpp — calc_subtraction_mismatch_hint needed a reflexive first step to stay a ≥2-step chain and keep its diagnostic); docs retaught incl. stragglers the spec missed (style.md raw-CIC table + claim-by-calc rule, structures-and-inference.md, internals/contexts.md, TODO.md). (b) DE-REWRITE DONE (761563b9 conversions + 83dc2c77 surface retirement): all 41 sites / 17 files converted — chain steps `by rewrite(L)`→`by L` (symmetric uses too, no Equality.symmetry wrapper), term transports → stated equation + bare transported proposition, nested rewrites → relation chains, absurd-wrapped sites thread a NAMED transported fact (absurd's argument is type-inferring; braces don't elaborate there); TWO order-relation transports keep `by substituting` (the bare bridge covers equality diffs only — order-diff transport is the queued diff-walker extension, would kill both). Surface retired at dispatch.cpp with an elaborate-level migration error (`rewrite` was never a lexer keyword); desugarRewriteTerm deleted (~350 lines, dispatch was its only caller); desugarRewrite (1-arg) SURVIVES as the internal citation-parity fallback (calc.cpp, inference.cpp); rewrite_term_test + transitivity_rewrite_test deleted; ErrorTest rewrite_retired added (error-tests now 47). (c) Rational.algebra:44 FIXED AT ROOT (9277c94e): quotientSoundBridge HOISTED from last-but-one to the cheap shape-gated tier — profiling showed the 615k steps were contextEqualityBridge's FAILED search (4.9 s) plus symmetryFlip fan-out (81 prover invocations) before the µs-scale bridge fired; module prover time 18.6s→143ms; warning-site diff exactly {Rational.algebra:44 removed}. The hoist EXPOSED a latent bridge bug: the wrap's inferred type is spelled at the peeled class_of form, which across an opaque quotient alias (Natural.to_integer, Integer/embedding.math:68) is NOT defeq-acceptable to the declared-type check even though the peel's WHNF crossed the boundary — the tactic now verifies its wrap against the goal's own spelling and declines otherwise (the old last-position bridge would have emitted the same broken proof anywhere it fired first). Bonus fix: the MATH_PROFILE_AUTOPROVER twin was MISSING both quotient bridges and symmetryFlip (reported their wins as 'unresolved') — it now mirrors the real tactic order. (d) style residual stands: multi-statement `by { … }` hint blocks predate the sweep and stay. Recipe archived at memory/a1_migration_recipe.md. **ORDER-DIFF TRANSPORT DONE (2026-07-04 fifth session, 845ecaac): the contextFactDiffBridge tactic** — diffs the goal against a same-head in-scope fact position by position, bridging each differing node coarsest-first (bounded defeq probe, then ONE in-scope/library equation matched at the node itself, descending into application children only when the node doesn't bridge as a whole; a recurring diff pair shares a multi-hole motive in a single transport_proposition; head mismatch retries at both sides' WHNF — induction-refined goals arrive unfolded as `a < b ∨ a = b`). Positional diffing is the point: the all-occurrence context-equality rewrite mangled `2 ≤ 1` (goal `1` also lives inside `2`'s successor spelling). Wrap verified against the goal as stated (the quotientSoundBridge lesson); profiling twin mirrors. Killed BOTH de-rewrite `by substituting` residuals (IntegerMod/field, Algebra/euclidean_domain — the latter closed bare already, only its dead name dropped) AND both Test.auto_generalize_test:51/:56 cold-build warning sites (their `done` goals now close in the cheap tier instead of burning 489k/306k steps reaching contextEqualityBridge). Feature test Test/fact_diff_bridge_test; docs: calc-and-rewrite.md order-relation paragraph retaught. Cold gates: library+tests+error-tests exit 0; warning-site diff = {auto_generalize_test:51, :56 removed}, the 15 library sites unchanged. Remaining A1-adjacent follow-up: the C6 breadcrumb sweep (row 2 residuals).** |
| 4 | B4 order automation in calc | **v1 DONE** | Landed 2026-07-02 night: monotonicity index (same-head order conclusions, bare-binder order premises = structural-descent admission, registerAlgebraicShape funnel, per-key ordered alternatives) + tryMonotonicityRecursion after the sign tactic at both prover hooks. v1.1 same night: two-sided keys (lhs-head-or-star, rhs-head-or-star) admit one-sided (x < x+e), constant-sided (negate(x) < 0), and mixed-head (triangle inequality) rules; sign-shaped premises follow the sign-index admission. Measured total: closes-today 463→743 (+280 sites, 12.0%→19.2%), B4 bucket 308→118; wall at baseline both steps. Residue (118): suspects are cross-relation weakening (le rule at lt goal), reflexive-leaf premises (a ≤ a), multi-position diffs needing two premises on the same head, and casts (tier-3 territory). The Natural strict add-monotonicity gap is CLOSED (add_left/right_strict_monotone, automatic; nested strict steps close by-less). **FRESH BASELINE + CAST-BIT TIGHTENING (2026-07-11, ec00273e — the post-F-queue re-measure the F1/F2 note queued):** the classifier's cast bit no longer trips on coercion towers bottoming out in a ground numeral (ε/4's tower spelling is spelling, not cast reasoning; isGroundNumeralTower in emitHintClassification). Numbers at the post-sweep tree (build_classify cache, `MATH_CLASSIFY_HINTS=1 make BUILD_DIR=build_classify library`, report via scripts/hint_classification_report.py): **4183 hinted sites** (sweep consumed ~250), closes-today **1315/31.4%**, tier3-cast **520→409** with the tightening (−111 numeral-spelling sites), tier3+4-sign-cast 70→30, tier4-sign 143→**184** (the released sites are really sign judgments — B4/tier-4 territory), tier2-ground 182, **B4-order-step 111** (top hints: Real.add_preserves_LessThan 24, add_preserves_LessOrEqual_left 13, add_preserves_LessOrEqual 12, triangle_inequality 9, multiply_monotone_nonneg 6 — consistent with the residue suspects: cross-relation weakening, reflexive leaves, multi-position diffs). Emitter fix riding along: classify goal-text truncation now cuts at a UTF-8 boundary (a mid-codepoint cut made text tools binary-flag the log). Gates: full re-verify + tests green, error-tests 51/0, site diff clean. B4-residue forensics can start from these numbers. **B4-RESIDUE FORENSICS + MECHANISMS LANDED (2026-07-11, 96fd531a): 89/111 bucket sites close at the 10k budget (was 53).** Instrument note first: `--check-redundant-by` NEVER probes order-relation calc steps — the non-eq gate is a separate `--check-redundant-by-non-eq` flag (a first probe run without it produced a zero-overlap artifact). The 58 true decliners split into two dominant families + a content tail: (a) **subtraction-spelled endpoints** (~30 incl. echoes in the ≤/Integer/Rational hint groups) — `x − z < y − z` is defeq to the `+`-stated rules but the index matches structurally; (b) **∧-leg premises** — the needed `d < q` lives inside `-q < d ∧ d < q` from an absolute-value bounds split, and the recursion's premise scan only read whole binders. Fixes in tryMonotonicityRecursion: stated spelling FIRST then retry at the add∘negate unfolding (unfoldSubtractSpellings — proof defeq-checks at the stated goal; the stated-first order matters: rules genuinely stated over subtraction like `x − ε < x` must keep matching, an unconditional normalization regressed intermediate_value's `c − δ < c`), and premise discharge falls back to collectLocalBinderFacts' ∧-leg projection gated hard (lazy collection + stated-head prefilter + structural-before-defeq — the recursion also runs inside the cast order tier's bounded prover, and an ungated scan starves downstream tactics via the shared budget). The 22 survivors: ~13 content citations correctly on the page (tailClose/boundFromL1-class local facts with data args, abs/modulus_partialSum_le bound lemmas, LessOrEqual.transitive), a 5-site Real.LessOrEqual.multiply_by_nonneg_right family, 4 scattered onesies. Gates: full re-verify + make check green, warning-site diff EMPTY, error-tests 51/0. **CONSOLIDATION SWEEP + THE MULTIPLY FAMILY CLOSED (same day, e6ecff5f + cf00c849): the B4 bucket's mechanism-absorbable residue is now FULLY closed.** Sweep: 269 order-step flags across 39 manifest modules (the class every prior sweep was blind to — non-eq steps only probe under the separate flag) judged in 4 batches: ~223 routine monotonicity/sign-flip/halving hints deleted, 46 keeps (IH, triangle-inequality, key-bound-lemma and data-instantiated-fact citations, case-defining branch inequalities), 7-name cascade settled; net −94 lines over 34 files. Multiply family root cause: the mono premise path handed premises to the sign recursion ONLY inside the order-judgment parse branch — a pure sign FORM (`IsNonneg(c)`) skipped both recursions and fell to raw defeq, where IsNonneg(c) vs `0 ≤ c` misses by the ring gap x−0=x. Fixed (non-order premises that parse as sign judgments go to trySignJudgmentRecursion; the automatic IsNonneg_of_LessOrEqual_zero form bridge crosses the spelling); Test/mono_sign_premise_test pins both hypothesis spellings; the 5 family hints deleted. What remains hinted in the old bucket is content citations only. Gates: make check green, error-tests 51/0, standing warning set unchanged. |
| 5 | A2 statement addressability + A7 `contradiction` kit | **A2 stage 1 + transport done; A7 done** | A2 stage 1 (2026-07-02 night): propositions in FUNCTION position and as `choose` sources address in-scope facts — desugared onto `given(...)`, which already implements the A2 semantics verbatim (defeq match, loud ambiguity naming both facts, anonymous facts participate). Discovery: A2's expected-type half already existed as the bare-proposition-as-proof coercion, so stage 1 closes the no-expected-type positions. By-cases transport LANDED (2026-07-02 night): given()'s failed defeq scan retries each binder through tryDiffBridgeViaContextEquality (the coercion path's existing single-position equation bridge) — a cases-with-eq arm addresses P(pattern) and the outer P(scrutinee) transports silently, same ambiguity rule. v1 scope: single-position diffs, equation must be in scope (cases … with eq); multi-occurrence motives and auto-synthesized equations stay `refining` territory. A7 DECIDED (delegated, 2026-07-02): skip the `contradiction` terminal (redundant with the owner-built done-closes-absurdity idiom); the `from <fact>: <instance>` restatement form is the A7 piece to build when reached. `by`-position survey largely covered by context-discharge. **A7 `from <fact>: <instance>` LANDED (2026-07-03, 4120b7d):** proof-block statement `from h: P [as NAME];` — source fact named first, transformed statement on the page; thin parser sugar onto the claim wrapper with the fact as by-hint (a probe showed `claim P by h` ALREADY transports across context equalities via the A2 bridge, so no new elaboration path). Feature test Test/from_instance_test.math; reference.md row. With the contradiction terminal skipped by owner decision, A7's delegated scope is COMPLETE; the remaining A7 bullets (by-lemma whole-body plumbing, let-in-definition-bodies, module-local open, piecewise) stay future work. **CORRECTION: the from-form was REMOVED the next morning by owner verdict (38aea212)** — `claim P by h` already transports, so it was a distinction without a difference; A7 stays complete (both pieces resolved by removal/skip). **A2 stage-2 SURVEY DONE + design scoped (2026-07-11; see the A2 section's "Stage-2 survey" block):** position coverage is COMPLETE (by/recalling/substituting take propositions; by-cases sources via prover; plain-split arms refine via auto-generalize; equation-arm transport handles multi-occurrence + nested diffs — the "single-position v1" description was stale, reference.md corrected + pins added to case_transport_test). Two build items decided: (1) ∧-leg participation in given() (probed gap: choose/function positions can't address conjunction legs; the prover already decomposes, so only those positions bite); (2) citation conclusion ∧-projection (`P(m) by h` with `h : ∀k. P(k) ∧ Q(k)` fails — mirrors the row-9 Not-peel). Deferred with reasons: symmetric-equality addressing (no demand), ∀-distribution (instance-level projection suffices), cast-normalized matching (no site), and the B1 statement-hash spine + blackboard (NOT built — no measured motivation; see row 2). **BOTH BUILD ITEMS LANDED (2026-07-11): A2 stage 2 is DONE.** (1) elaborateGiven scans `collectLocalBinderFacts` — the prover's decomposed view (base binders + recursive ∧-legs with projection proof terms) — in TWO TIERS: whole hypotheses (cost 1) first, legs only when no whole matches, so a leg duplicating an exact hypothesis cannot create new ambiguity and pre-change behaviour is preserved verbatim; within a tier two matches stay the loud error, now naming facts via their source strings ("local binder h (∧-left)"); the transport retry runs over the same list, so legs transport through case-arm equations too. (2) autoFillHintForClaimCore grew an And-conclusion projection block after the Or.self collapse (same precedent family): a leg unifying with the goal completes the citation for the whole conjunction, reads the instantiated legs off the completed call's type, and wraps the matching And.left/right — gated OFF the speculative context scan (prompted citations only; the unprompted pool's admission prefilter never admits And-conclusions for leg goals, so scan cost and power are untouched). Feature tests conjunction_leg_address_test (choose + function positions, nested legs, tier rule, leg transport) + citation_conjunction_projection_test (both legs, premise discharge, whole-body `:= by h`); ErrorTest statement_address_leg_ambiguous. Gates: make check green, error-tests 52/0, redundancy warning-site set UNCHANGED (181 = 181 sites, full non-eq sweep both sides), export-check 2554 PASS. **Consolidation sweep landed same day (net −77 lines across 11 files):** every library `given(…)` wrapper dropped (bare statements at choose sources + argument positions; `N ≤ N` reflexivity breadcrumbs deleted — the argument coercion proves them); unused-leg `∧`-statements folded onto the new projection (`-ε < d by absolute_value_LessThan_bounds` states only the bound used — supremum ×4, IVT ×2, linearity ×2, limits ×2, cauchy_complete ×2, order ×1; cauchy_complete:160 kept, it uses both legs); restate-then-choose double-statements folded to `choose … such that … from <lemma>` (archimedean ×3, supremum ×5, multiplication's product_is_cauchy block −21 lines incl. both stale "obtain cannot infer" apology comments, order_field ×2, IVT, order_multiplication). Manifest warning sites unchanged/improved (archimedean 4→3). **FOUR choose-form elaborator gaps hit and routed around (backlog, with reproducers in this sweep's reverts):** (α) multi-witness choose rejects `such that … from <lemma>` (message suggests the claim-first form; such-that was load-bearing for premise disambiguation — s-vs-t IsCauchy); (β) `choose … such that … from <applied hypothesis>` fails with an anonymous-tuple expected-type error (order.math sumNonneg site, reverted); (γ) choose-from-lemma's witness-type reader declines CauchyRationalSequence.isCauchy's conclusion (linearity, reverted; message suggests claim-first); (δ) bare-proposition ARGUMENTS in a choose source (`sNonneg(δ, (0 < δ))`) fail inside pattern-match-form theorems via the obtain path ("expects a lemma name") but work in plain theorems (multiplication vs order_multiplication). **GAPS FIXED same day (α, β, δ fully; γ's reader half): every diagnosis differed from the sweep-day guess — the fixes, each at its own root:** (β) TWO defects — the destructured constructor value-arg binder types in buildCaseLambda were never `deepBetaReduce`d (unlike the motive and recursion-hypothesis right next to them), so a choose from an applied definition-typed source (`sumNonneg(ε, εPositive)` where IsEventuallyNonneg instantiates `f(m)` at a lambda) bound the pred hypothesis at a spelling with INNER β-redexes that the arm's order-step matchers can't parse; AND elaborateBlockTail's statement-reading fallback masked the real error behind "anonymous tuple needs an expected type" (a `witness` tail desugars to a tuple — now in tailIsProofOnly, so the direct reading's error surfaces). (δ) also two defects — the choose source PROBE swallowed its exception and misrouted applied sources to CiteInferred's "expects a lemma name" (now: non-identifier sources rethrow the probe error), and the REAL failure was the bare-prop-as-proof coercion's prefilter demanding STRUCTURAL equality, missing defeq-equal reduced premise spellings (`to_rational(to_integer 0) < δ` vs the written `Rational.zero < δ` — instantiated theorem premises arrive δ-unfolded); prefilter widened to same-relation-head admission, full defeq still gates the action. (α) the such-that lemma branch generalized to MULTI-WITNESS: one ∃ layer read off the conclusion per witness name, premise peel iterates through WHNF (also the reader half of γ — conclusions spelled through definitions expose their buried Pis and ∃s). En route: autoProveCalcStepRaw's strategy-1 defeq probe guarded against TypeError (a speculative check must decline, not throw — the defeq recursion re-types manufactured subterms across the soft-opaque alias boundary). Library acceptance: order.math sumNonneg one-liner (β), order_multiplication bare-prop source + TWO multi-witness one-liners (δ+α); feature test choose_from_lemma_test pins α and the β-redex shape. **Residuals, precisely bounded:** (γ second half) citing a fact whose ∃ sits behind definition-buried PREMISES (isCauchy's ε, εPositive — holes exist only for syntactic Pis) still can't instantiate them — linearity keeps the claim-first form; (multiplication.math steps 1-2) the α one-liner at THAT site passes elaboration but fails the final whole-declaration kernel check — a by-less winner synthesizes a reduced-spelling proof term (`Quotient.lift(<unfolded multiply>)(t m)`) under the changed context — the (b)-class opacity fragility from the D-plan history, banked with its backtrace signature (rethrowKernelError ← elaborateDefinition final check); site kept in the ∃-then-multi-witness two-statement form. **∨-SYLLOGISM CLOSE-OUT (same day): the "B-workstream gap" from the A4 lint triage was ALREADY CLOSED — tryDisjunctiveSyllogism landed 2.5 hours after the triage note (B3.3, 3b22980b) and the note was never revisited; the real residue was the un-folded SITES.** Done now: (1) findRefutation gained a GROUND-TIER fallback (a numeral side like `(1 : Natural) = 0` refutes by decision, search-free — the shape euclid-class divides-alternative splits need); (2) the contradiction-only-arm lint gained the env-gated unscoped mode `MATH_LINT_CONTRADICTION_ARMS_ALL` (the sweep instrument; 39 firings library-wide, 32 in the manifest); (3) **11 of the 32 manifest sites folded** (me ×2 + 3 Sonnet agents: order lt_of_le_of_ne → bare `done`, euclid, divisibility, prime_divides_product, associate, both integral_domains, list, excluded_middle double_negation_eliminate, minimum, Real.linearity not_LessOrEqual_implies) — each states the survivor bare where the split stood; **the 19 keeps are PRINCIPLED**: every one's refutation is a genuine multi-step derivation (weaken-then-contradict, substitute-then-match, 2≤p-from-primality) — content that belongs on the page, exactly the boundary the tactic should have. Feature pins added to disjunctive_syllogism_test (≤-unfolds-to-∨ whole-proof `done`; ground refutation). **And-admission for the unprompted pool: MEASURED AND CLOSED as won't-build** — exactly 2 automatic ∧-conclusion lemmas exist library-wide (not_implies_implies_and_not, LessOrEqual.invert), so admission would add two candidates for a per-declaration-per-goal peel cost; the prompted projection covers citations. Gates: make check green, error-tests 52/0. |
| 6 | A3/A4/A5 construct distillation | **A3/A4/A5 DONE** (A4 closed 2026-07-07: multi-binder equation-shaped induction arms) | **A3 (2026-07-04): one `for` grammar for both suppose exits** — `suppose P for proving Q { … }` replaces `to prove Q` (migration error; one real use, the feature test), and the ∀-analog `take x : T for proving Q { … }` landed (unrolls through the same wrapper — a lambda over the data binder gives the Pi statement in context; feature tests in suppose_modifiers_test). **`suffices … by definition of X[, Y]` LANDED (2026-07-04)**: Q proves under an unfold wrapper carrying the definitional step (works through opaque definitions; feature test suffices_by_definition_test). Found + fixed while landing it: the A1 bare-statement catch-all had SHADOWED the post-loop `suffices`/`apply` handlers since stage 1 — both constructs were dead in the grammar (zero live library uses masked it); the loop condition now excludes them. `suffices` also entered reference.md (it was never documented — C4 gap). Still open from A3: the block-length announcement lint. **A5 (2026-07-04, in flight): multi-witness choose landed** (`choose m, n such that P [from S]` — the n-ary tuple pattern flattens nested ∃/∧; feature test choose_multi_test; the lemma+such-that disambiguation path rejects witness lists with a claim-first pointer), **the `representative` pattern landed** (`take x as representative(a, b) : Integer` / `cases x { | representative(a, b) => … }` — quotient-cases routes a constructor pattern named `representative` through the tuple path, so the carrier's constructor name leaves the page; take_with_pattern_test updated), and the **obtain→choose sweep is DONE (484cac50): `obtain` is RETIRED** (parser migration error; ~194 sites / 60+ files converted — choose for ∃/∧ with the property stated inline, `let ⟨⟩ :=` for genuine data, bare `as`-restatements for named conjunction legs, plain deletion for pure-∧ destructures of in-scope hypotheses; obtain_* tests renamed/repurposed; ErrorTest obtain_retired; docs rewritten across LANGUAGE.md/reference/tutorial/proof-style/quotients). Wave findings: (α) a by-less calc that rewrote through a leg the OLD tuple named works only if the equality's spelling matches the goal's — the anonymous ∧-leg keeps the LEMMA's spelling (e.g. IntegralDomain.ring(d) vs the let-alias s), and tryContextEqualityBridge matches occurrences structurally; remedy = restate the equation bare at the working spelling (principal_ideal_domain, 2 sites) — a candidate for ζ-tolerant occurrence matching later; (β) choose's unused-name warnings caught 6 sweep-introduced dead names, settled. A5 is COMPLETE (multi-witness choose, representative pattern, obtain retired, `choose` the sole logic eliminator). | 2026-07-04: **`decide` is deleted from the proof surface** (parser throws a migration error steering to `by cases`/`if`; `decide` stays a contextual keyword for qualified names like Natural.decide). The two-phase recipe finished in one session: (1) QUARANTINE — generic `Logic.if_positive.{u}`/`if_negative.{u}` in Natural/classical_decidable.math (`(if P then a else b : T) = a/b`; the file joined scripts/foundational_layer.txt — it IS the classical-decidability machinery) proved with the `if` spelling itself; every refinement-class definition then publishes one-liner characterizing equations (`by Logic.if_positive`) — the citation matcher unfolds min/filter/bisectionStep through their WithDec helpers and matches the conditional shape. Rational/minimum (pilot, now one-liners), Lists/filter (filter_prepend_positive/negative), Real/supremum (bisectionStep_eq_of_[not_]upper_bound + the FOUR endpoint recurrences bisectionLeft/Right_successor_of_[not_]upper_bound — all invariant proofs reason through them). (2) SWEEP — ~40 proof-side sites across Polynomial/ComplexNumber/Real/Algebra/Lists/Natural/Rational converted to `case P: … otherwise: …` (5 Sonnet agents + orchestrator; wave-level build as verification per the June workflow). Value-level definitions keep `if P then a else b` (the sole surviving spelling; docs updated: LANGUAGE.md, reference.md, tutorial.md, proof-style.md). Substitution-rule fallout: 2 arms got SIMPLER (degree_product `case i = m: done`, bezout speaks zero directly). ELABORATOR FIXES flushed out by the sweep (commit d823bd43): (α) both structured-claim-arm CLONE sites (substituteSurfaceName, rewriteRecursiveCalls) dropped isOtherwise/witnessName/witnessType — any otherwise-arm under a `let`/`set` or in a recursive definition became a stated arm with a null proposition and SEGFAULTED; clones now copy fully + a loud internal error replaced the null deref; (β) `P by substituting <Lemma>` now discharges the lemma's Prop premises via the budget-capped bare prover (mirrors citation premise discharge — needed for premised characterizing equations); (γ) the endpoint-instance scan falls back to CONTEXT-FACT statements when the goal carries no instance (hypothesis-transport claims state the rewritten form). Tests: decide_test/decide_recursion_pi_test → if_split_test/if_recursion_pi_test (given(P) addresses the anonymous branch hypotheses); ErrorTests decide_retired + if_missing_classical_decidable; 37/37 green, library + both ratchets green (manifest floor stays 204). A3/A5/induction-as-by-cases still open. **`cases…with eq` RETIRED (2026-07-04 second session, c3afb8c1 + b053c7c7):** the 2 divide.math sites moved to equation-shaped by-cases (Opus agent, diff reviewed — both scrutinees absent from their goals, so the old refinement was a no-op and the bodies carried verbatim; the `for some (predM : Natural)` witness + `as monusEq` feed the same *_succ_fits preconditions); the grammar throws a migration error (internal CasesWithRefining plumbing stays — equation-arm desugaring produces it with an empty name); tests repurposed (case_equation_split_test, case_transport_test on an equation arm), ErrorTest cases_with_eq_retired, docs reteach across LANGUAGE/reference/tutorial/proof-style. **The two acceptance-form textual deltas CLOSED (22623af7, Opus agent, reviewed):** `take ε > 0;` (desugars to take + anonymous suppose; 5 relations; multi-binder rejected, ErrorTest) and `P by V, by definition of X[, Y];` (wraps the WHOLE claim in the suffices-by-definition SurfaceUnfold — the proposition-vs-goal check sees through the definition, unlike postfix `unfolding` which only unfolds the hint proof). **BOTH A4/A7 kit chunks LANDED (2026-07-04 third session; Opus agents against written specs, diffs reviewed line-by-line, gates re-run by the orchestrator). Multi-binder witness arms (f36f61ea):** `case P for some k, j, …:` — SurfaceWitnessBinder list replaces witnessName/witnessType (field DELETION made the compiler find every walker; surfaceMentionsName also gained the missing binder-type scan, with earlier-binder shadowing); disjunct = nested ∃; un-annotated binder types infer by priority annotation > rhs-constructor Pi-domain at the bare-argument position (dependent domains demand witness-free earlier args, hard error says annotate) > lhs carrier; arm opening is now a LOOP over Exists.eliminate with the derived lift tables (body cutoffs 2,4,…,2n; Tᵦ 0,2,…,2b; nest 1,3,…,2b+1; goal 2b+1; scrutinee BV0 each level) — n=1 reproduces the old term exactly (whole library re-verified unchanged); A4 substitution keys on left-endpoint index ≥ n, goal lift n+1. Feature tests incl. telescope inference + a dependent (carrier, element : carrier) pair; ErrorTest witness_duplicate_name. FINDING: proof-valued dependent witnesses (`for some (k : Natural), (h : k ≤ n)`) are NOT expressible — Exists demands a Type-valued witness (universe-predecessor of Sort 0 fails); a real library limitation to remember. **Constructor-disjointness refutation (c977a734):** tryConstructorDisjointness (prover.cpp, cheap tier after the disjunctive syllogism, both hook sites) — False goals close from context equalities whose endpoints WHNF to DISTINCT constructors of one Type-valued inductive; discriminator D : T → Prop via the recursor (arm arities read off the recursor's own Pi telescope; motive body Sort 0 at motive level 1), True.trivial transported across the equation; Prop-valued inductives decline via the missing motive-universe parameter; opaque heads (`1 + n`) stay stuck BY DESIGN (rewrite via one_add first). Wall-neutral (33.5→33.3s), warning sites unchanged. Test/constructor_disjointness_test (List + Natural + numeral + by-cases-arm shapes). Simplification candidates left UNSWEPT for a polish pass: Natural/strong_recursion:60, prime_split:224, divisibility:163+170, Lists/distinct:249 (successor_not_zero/zero_not_successor citations `done` now closes alone). |
| 7 | A6 `eventually` | **DONE incl. the analytic acceptance target (SequenceIsCauchy two-index tail deliberately outside)** | 2026-07-04: library substrate + binder form. `Natural.Eventually(P) := ∃N. ∀m ≥ N. P(m)` (Natural/eventually.math) with the elaborator-facing lemma set: `Eventually.and` (∧-closure via maximum — the invisible-threshold combinator), `Eventually.monotone` (pointwise weakening), `of_always`, `past` (re-threshold past any later index). Surface: `eventually (m). P(m)` parses as an ordinary proposition (contextual `eventually`, claimed by the `(name).` lookahead shape only — the word stays usable as an identifier); rule (iii) came free (`choose N such that ∀m. N ≤ m → P(m) from h` opens the threshold — Eventually is transparent, WHNF exposes the ∃). Feature test eventually_test (states/combines/weakens/choose); reference.md row. **Rule (ii) LANDED same day: `eventually (m): { … }` in goal position** — new SurfaceEventuallyScope (parser lookahead `eventually (name):`, distinct from the binder form's `.`; all four surface walkers extended — substituteSurfaceName also gained the missing SurfaceByStrongInduction case while at it); the elaborator collects every in-scope `Natural.Eventually(Pᵢ)` fact (STATED-spelling head match — WHNF would δ-unfold the transparent definition past recognition, the day's recurring lesson), folds them with `Eventually.and` (combined predicate mirrors the lemma's conclusion shape exactly, applications unreduced, so the chain typechecks structurally), and closes with `Eventually.monotone` handing the body `m` + the combined hypothesis (∧-legs decompose for the prover); zero eventual facts degrades to `of_always`. Tests: scope combining two hypotheses, derivation body, always case — thresholds never on the page. **The Real-ε bridge LANDED same day: `Real.SequenceConverges` and `Real.TendsToInfinity` are now DEFINED through the eventually binder** (`∀ε > 0. eventually (m). abs(s(m) − limit) < ε`) — Natural.Eventually's body is exactly the old ∃N.∀m≥N shape, so the restatement is definitionally transparent: total library fallout was ONE proof term (at_rational's `done` became the explicit instantiation — the by-less prover doesn't defeq-hop through the new head) and TWO interface imports (Natural.eventually joins the public vocabulary; the transparent-export closure validator caught it — the D-machinery working as designed). Every convergence goal now opens with the eventually scope. STILL OPEN: Real.SequenceIsCauchy keeps its two-index tail shape (not an Eventually instance); `for sufficiently large m: { … }` prose sugar LANDED (same scope node). **THE ACCEPTANCE REWRITE LANDED (2026-07-04): `Real.LessOrEqual_of_pointwise_lower` is now the plan's analytic reference form in the library** — `take B as representative(b, bIsCauchy); suffices ∀ε. ε>0 → eventually (m). -ε < s(m)-b(m) by definition of Real.LessOrEqual, Real.IsNonneg; …; eventually (m): { calc -ε = … < … ≤ … = s(m)-b(m) }` — and the 60-line at_make helper + wrapper pair is DELETED (consumer-free). Pieces landed to get there: (1) `Rational.sequence.IsEventuallyNonneg` restated through the eventually binder (fallout: ONE claim site, order.math — matching the SequenceConverges precedent); (2) **type-less `take x as <pattern>;` destructures an IN-SCOPE binder in place** (parser TakeDestructure wrapper → cases; auto-generalize refines dependent hypotheses); (3) `Rational.sequence.IsCauchy.eventually_near_anchor` (sequence.math) — the anchored-modulus lemma whose ∃-body IS the choose's stated property, so `choose N such that eventually (m). |s(m)-s(N)| < ε/2 from …anchor` premise-discharges; (4) `Real.eventually_below_cast_of_LessOrEqual` (embedding_order.math) — one-line boundary lemma (`(unfold Real.IsNonneg in belowCast)(ε, pos)`) consuming `mk(b) ≤ (q:Real)` into the eventually vocabulary; the probe's citation premise-discharged from the ∀-fact pointwiseLower AT N automatically; (5) **the eventually scope + fact admission now FOLD BACK δ-reduced spellings** (recognizeUnfoldedEventually: the raw ∃N.∀m≥N shape refolds to its predicate — quotient-cases motives/suffices ascriptions hand the scope reduced goals) **and peel β-redex fact types** (choose's elimination motive leaves `(λN. eventually …)(w)`). Textual deltas from the plan's exact ideal, recorded: `∀ (ε : Rational). ε > 0 →` for `∀ ε > 0.` and the two-statement `take ε; suppose ε > 0` for `take ε > 0;` (combined-header sugar NOT built yet), `from …eventually_near_anchor` for `from sIsCauchy` (the anchoring derivation is a lemma, honestly named), `by …eventually_below_cast_of_LessOrEqual` for `by pointwiseLower(N), by definition of …` (comma-joined by-definition claim modifier not built), Rational.halve(ε) for ε/2 (the halve→/2 sweep is old backlog), one stated abs-bounds ∧ + one add-mono `by` inside the scope (prover-strength residuals, same class as sqrt's two hints). |
