...
| 4 | B4 order automation in calc | **v1 DONE** | Landed 2026-07-02 night: monotonicity index (same-head order conclusions, bare-binder order premises = structural-descent admission, registerAlgebraicShape funnel, per-key ordered alternatives) + tryMonotonicityRecursion after the sign tactic at both prover hooks. Measured: closes-today 463→623 (+160 sites, 12.0%→16.1%), B4 bucket 308→181; wall at baseline. Residue: the 181 remaining order-step sites (audit what shapes they are — cross-relation le→lt weakening, reflexive-leaf premises, and mixed-carrier casts are the suspects); a strict add-monotonicity lemma is MISSING at Natural (library gap). |
