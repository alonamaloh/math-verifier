...
- `S3.C1` `[~]` (M) ★ The set of truants occurring anywhere in the tree is exactly **{1,2,3,5,6,7,10,14,15}**. **CENSUS COMPLETE BY RANK 2026-07-24** (`Algebra/fifteen_theorem.math`): ranks 0–3 were already classified in `Algebra/critical_truants`; `Matrix.escalator_rank_four_census` proves every rank-four escalator universal or of truant 10/15 (conditional on the seven converse interfaces, via the enriched outcome dispatch carrying `IsTruant` certificates), `Matrix.escalator_rank_five_universal` proves every rank-five escalator universal (parent exceptional ⇒ the generic `escalation_universal_of_parent_representsEveryPositiveExceptTruant`), and `Matrix.escalator_beyond_rank_four_universal` closes all higher ranks by induction (a parent that is universal admits no escalation). Remaining for the "exactly": the ⊇ occurrence side (each critical value is attained) wants `IsEscalator` membership certificates for the classified representatives — elementary, not yet stated. — owes: S3.E3, S3.E4, S3.E5, S5.U2, S5.U3
  ...
  **LOW-RANK AND EXCEPTIONAL FINITE CASES COMPLETE 2026-07-21:**
  ...
  The through-15 certificates for all 201 forms have now been upgraded to
  conditional universality proofs. `PROOF_FIFTEEN_THEOREM_ASSEMBLY.md`
  supplies the remaining mathematical escalation argument: each of the six
  exceptional forms represents every positive integer except its truant, so
  every further escalation is universal. The corresponding `.math`
  certificates remain to be emitted, as do the explicitly named ternary
  converse interfaces used by the 201 proofs. In particular, rank four can
  repeat the old truant 10; the earlier claim that every non-universal
  rank-four form must have truant 15 was too strong and is corrected here.
- `S3.C2` `[x]` (L) ★★ **Master reduction:** if `Q` represents the nine critical numbers then `Q` is universal — via: a non-universal `Q` contains an escalator with the same truant. `PROOF_FIFTEEN_THEOREM_ASSEMBLY.md`, Section 6, gives the complete maximal-chain proof, including the same-rank integral-overlattice steps allowed by Bhargava's definition and their determinant descent. **FORMALIZED 2026-07-24** (`Algebra/escalator_embedding.math`, `Algebra/escalator_exclusion.math`, `Algebra/fifteen_theorem.math`): `Matrix.universal_of_critical_values` builds an escalator-embedding chain inside the ambient form (`Matrix.IsEscalatorEmbedding`: independent columns whose Gram matrix is an escalator; `Matrix.appendColumn` with its action and bordered-assembly Gram laws). The same-rank overlattice case is **refuted rather than modeled**: a rational dependence of the new truant vector on the chain scales through the adjugate (`Matrix.adjugate_applyVector_left`, using positive-definite injectivity) to an integer vector representing det(E)²·truant(E) on the escalator E itself, which the per-representative exclusion certificates forbid — so the chain stays inside the rank-increasing `IsEscalator` tree and Section 6.1's separate ambient-lattice API is unnecessary. Rank-one/two exclusion certificates are proved (`escalator_rank_one_exclusion`/`_rank_two_exclusion`, via `¬Represents(x², 2)`, `¬Represents(x²+y², 3)`, and a new `¬Represents(x²+2y², 20)` finite check). **THE NINE RANK-THREE CERTIFICATES ARE PROVED 2026-07-25** (`Algebra/rank_three_exclusion_reductions`, `Algebra/rank_three_exclusion_descents`, `Algebra/rank_three_exclusion`): `Matrix.escalator_rank_three_exclusion` dispatches along the ternary classification, and the interface `Matrix.RankThreeEscalatorExclusion` is deleted rather than kept as a discharged wrapper. Five representatives reach Legendre's obstruction through the parity identities already in the library — 7 is the truant itself, 2·56 = 112 = 4²·7, 112 is a three-squares value outright, x²+2y²+4z² = 896 is an x²+y²+2z² value with 2·896 = 1792 = 4⁴·7, and odd(5) has the three-squares represented set with 567 = 8·70+7. The other four descend by a prime the form can only absorb coordinatewise: 54 = 9·6 by two descents modulo three; 1000 = 25·40 modulo five, leaving z² ≡ 3; 360 by the classes modulo sixteen, which make every coordinate even and then refuse the quarter 90 outright; and 343 by doubling to 2x² + (2y+z)² + 7z², which puts seven into every coordinate and lands the forty-ninth part on the truant. The residue and square-class arithmetic at moduli 5, 7 and 16 is generated (`scripts/generate_residue_arithmetic.py`, wired into `residue-arithmetic-generated-check`); the generator derives the surviving square classes itself rather than taking them on trust. The representatives' determinants are proved alongside — the odd border's via its Schur complement, whose adjugate-border pairing is pinned by the adjoint law without ever computing the adjugate. — owes: S3.C1, S3.E5, S5.U2
...
- `S4.S*` `[~]` (M each) ★ For each ternary section `T_i` from `S3.E5` (`x²+y²+2z²`, `x²+2y²+2z²`, `x²+y²+3z²`, …): its exact represented-set, via **(a)** `IntegerMod/` congruences (escape hatch), **(b)** `S4.G2` regularity, or **(c)** direct argument. *(Tag each section with the route it actually needed.)* — owes: S4.L3, plus per-section S4.G2 / `IntegerMod/` · **PARITY REDUCTIONS DONE 2026-07-21:** `Algebra/three_squares_parity_transforms` proves integrally that `x²+2y²+2z²` has exactly the same represented integers as `x²+y²+z²`, and that `x²+y²+2z²` represents `n` exactly when three squares represents `2n`. The proof divides three coordinates modulo two, selects a same-parity pair, and applies the identities `(u+v)²+(u-v)²=2u²+2v²` and `(x+y)²+(x-y)²+(2z)²=2(x²+y²+2z²)`. Both ternary represented-set problems therefore reuse `S4.N1` and need no independent genus argument; the rank-four forms above them still require their residual/congruence covers. **ONE-TWO-FOUR CONVERSE REDUCED 2026-07-22:** `Algebra/one_two_four_converse_reduction` proves `Matrix.one_two_four_converse_of_three_squares`. If `n` avoids `4^a(16b+14)`, then `2n` avoids Legendre's `4^a(8b+7)` obstruction. A three-square witness for `2n` descends first to `a²+b²+2c²=n`; an integral parity transform then makes one coefficient-one coordinate even and rewrites it as `x²+2y²+4z²=n`. Thus this Ramanujan–Dickson converse is not an independent representation-theory or genus obligation: it follows from `S4.N1` by elementary identities. **ALL-AT-ONCE CONVERSE CENSUS 2026-07-23:** `PLAN_TERNARY_CONVERSES.md` classifies every current ternary input by determinant, bad primes, exact obstruction, client count, and proof route. `Algebra/odd_five_converse_reduction` proves that `x²+2y²+2yz+5z²` has exactly the same represented integers as three squares: its index-three identity is inverted by choosing signs modulo three. Thus the 12 odd-`C=5` forms need no new deep converse, although their rank-four covers remain to be assembled. **ONE-THREE-SIX REDUCTION IMPLEMENTED 2026-07-23:** `Algebra/one_three_six_converse_reduction` proves `Matrix.one_three_six_local_converse_of_three_squares`; the eight clients no longer add an independent genus input. Two-three-six remains an index-six reduction pilot. The determinant-seven odd parent is nonregular, so its 14 nonexceptional rank-four clients must use a restricted-coset or direct quaternary argument rather than a false blanket local-global converse. The census also confirms that the genus route for one-two-five is mathematically valid, while its Mordell route's restricted Dirichlet theorem may dominate the cost.
- **TRIPLE-SQUARES PROOF SPECIFICATION DONE 2026-07-23:** `PROOF_TRIPLE_SQUARES_CONVERSE.md` gives an end-to-end proof that `x²+y²+3z²` represents exactly the naturals outside `9^a(9b+6)`. The local analysis is explicit at every prime. Hasse--Minkowski supplies a rational point; equal-norm reflections and finite lattice patching put it in a genus representative. A two-dimensional Hermite bound and orthogonal projection force every positive determinant-three ternary lattice to represent one, reducing the class calculation to the verified binary pilot. The two resulting ternary classes are separated over `ℤ₂` by primitive representation of a multiple of four. Thus no general ternary reducer, mass formula, spinor genus, or Jones--Pall table is needed for this target. Hasse--Minkowski remains the single substantial classical input and must be budgeted honestly: standard proofs use Dirichlet or equivalent global reciprocity machinery.
...
- **WEIGHTED D4 ZERO-RESIDUE COVERS DONE 2026-07-22:** `Algebra/rank_four_weighted_d4_foundation` records the Ramanujan–Dickson converse for `x²+2y²+4z²`, whose exact exceptional shape is `4^a(16b+14)`.  The generic `Algebra/rank_four_weighted_zero_residue_universality` lift is parameterized by the third diagonal coefficient and will also serve the weighted-`d=5` branch.  Nine of the eleven selected diagonal extensions use the direct residual `4^a(16b+14-c)`.  For `c=6`, the proof uses scale `2^a` when `b=0` and `2^(a+1)` otherwise, leaving cores 2 and 6.  For `c=14`, it tests the inner parameter: an admissible `b` becomes residual `4^(a+2)b`; an obstructed `b` is written `7+t` with admissible `t`, and scale `3·2^a` leaves `4^(a+2)t`.  `Algebra/rank_four_weighted_d4_zero_residue_covers` aggregates all coefficients `c=4,…,14`, raising the selected conditional total to 96.
- **WEIGHTED D5 ZERO-RESIDUE COVERS DONE 2026-07-22:** `Algebra/rank_four_weighted_d5_foundation` records the Ramanujan–Dickson converse for `x²+2y²+5z²`, whose two exact exceptional shapes are `25^a(25b+10)` and `25^a(25b+15)`.  `Algebra/twenty_five_power_obstruction_arithmetic` factors the common power-comparison argument into a reusable affine-obstruction lemma.  Coefficients `c=6,…,9` subtract `(5^a)²c` and leave one of eight distinct nonzero cores.  For `c=10`, the `+15` branch leaves core 5; the `+10` branch uses residual 0 for `b=0`, core 1 for `b=1`, and scale `2·5^a` with core 20 thereafter.  The import-only aggregate deliberately omits `c=5`, already proved to have exact truant 15, and raises the selected conditional-universality total to 101.
- **ALL NONEXCEPTIONAL WEIGHTED D5 COVERS DONE 2026-07-23:** `Algebra/rank_four_weighted_d5_cover` proves the common bordered-family argument.  For an obstructed target `25^a(25b+q)`, first represent the base target `25b+q`, then scale all four coordinates by `5^a`.  At the base, each selected border has a fixed fourth coordinate that leaves an admissible one-two-five residual beyond a finite cutoff.  `scripts/generate_rank_four_weighted_d5_covers.py` emits 1,470 explicit, kernel-checked witnesses for the finite prefixes and a `--check` drift gate; the handwritten theorem proves every infinite tail.  The generated proofs are split at 20 rows per theorem and one form per module: the initial large-theorem rendering exposed the known quadratic local-context cost, while the bounded rendering verifies the two largest tables plus their all-28 aggregate in about 27 seconds.  `Algebra/rank_four_weighted_d5_covers` aggregates the previous five diagonal theorems with the new 28 bordered theorems.  Thus all 33 nonexceptional selected weighted-`d=5` forms are conditionally universal from `Matrix.OneTwoFiveConverse`; the other four are the already classified exact-truant forms.  The selected conditional total is now 175, leaving only the 26 nonexceptional odd-family forms among the 207 selected representatives.  **CONVERSE PILOT:** unlike one-two-four, no elementary reduction to three squares was found.  The known determinant-10 proof is a genuine Mordell-style argument requiring quadratic reciprocity, primes in specified progressions, and a finite determinant-10 class elimination.  `PLAN_ONE_TWO_FIVE_CONVERSE.md` records that route and the alternative gated genus route; keep the converse as an explicit deep interface until the remaining odd-family converses reveal which machinery is shared.
- **ALL ODD C5 COVERS DONE 2026-07-23:** `Algebra/odd_five_converse_reduction` first identifies `x²+2y²+2yz+5z²` with the represented set of three squares.  `Algebra/rank_four_odd_c5_cover` then handles `Q=F+2ryw+cw²` at the base obstruction `8b+7`: choosing a fixed `w` with `9 | rw`, writing `s=rw/9`, and applying `Q(x,y-5s,z+s,w)=F(x,y,z)+cw²-45s²` leaves an admissible ternary residual beyond a finite cutoff.  `scripts/generate_rank_four_odd_c5_covers.py` emits 586 explicit kernel-checked witnesses for the finite prefixes, one form per module and 20 rows per theorem, with a deterministic `--check` gate.  `Algebra/rank_four_odd_c5_covers` proves each of the 12 selected forms and the family predicate universal from `Matrix.ThreeSquaresConverse`.  **ALL DETERMINANT-SEVEN COVERS DONE 2026-07-23:** the first seven nonexceptional odd-`C=4` forms use the earlier sublattice and neighbor arguments. The final seven use `Matrix.DetSevenSafeConverse`, deterministic modulo-588 residual certificates, and 1,164 explicit finite witnesses. Six lift the determinant-seven ternary section through orthogonal complements of norms 7, 266, 280, 329, 238, and 287. The exceptional `(1,7)` form uses norm 315 for odd targets and the alternative doubled odd-five section with norm 90 for even targets, retaining `Matrix.ThreeSquaresConverse` as a second explicit input. `Algebra.det_seven_covers` collects all seven final theorems. The selected conditional total is now 201/201; the six other selected rank-four forms remain exact truants.
- **WEIGHTED D4 THIRD-DOUBLE COVERS DONE 2026-07-22:** For `squarePlusDoublePlusScaledRankFourRepresentative(4,0,2,c)`, the integral completion is `Q=x²+2y²+(2z+w)²+(c-1)w²`.  An `x²+2y²+4z²` witness already supplies an even completed coordinate, so it is compatible with `w=0` and with every positive 4-adic exponent.  At exponent zero, the six even selected corners leave odd residual cores; `Integer.odd_square_square_double_has_even_odd_order` swaps the coefficient-one roots to make the completed coordinate odd.  For the remaining cores 10, 4, and 2, `Algebra/rank_four_weighted_d4_third_double_parity` applies the integral norm-preserving transform `(a,2c,y) ↦ (a/2+c+y, a/2+c-y, a/2-c)` and proves the required output coordinate odd from the residual modulo eight.  The reusable base-control interface and one nine-theorem concrete module prove every selected corner `c=4,5,6,8,10,11,12,13,14` conditionally universal and raise the selected total to 110.
- **WEIGHTED D4 THIRD-UNIT COVERS DONE 2026-07-22:** `Algebra/rank_four_weighted_d4_coset_cover` uses the uniform identity `4Q=(4z+rw)²+2(2y+sw)²+4x²+(4c-2s²-r²)w²`.  In an obstructed target `m=4^a(16b+14)`, it represents an admissible base core first and scales the coordinates by `2^a`, eliminating all positive-exponent parity descent.  For `(s,r)=(0,1)`, the residual cores are all `1 mod 4`; a two-variable parity lemma makes the coefficient-one root `1 mod 4` up to sign and the coefficient-two root even.  `Algebra/rank_four_weighted_d4_third_unit_covers` proves all eleven selected corners `c=4,…,14` conditionally universal and raises the selected total to 121.
- **ALL WEIGHTED D4 COVERS DONE 2026-07-22:** The scaled base-coset method now covers the remaining 26 selected borders.  `Algebra/rank_four_weighted_d4_even_cosets` proves reusable controls for cores 2, 6, and 10 with affine residues `(0,1)` or `(2,1)`; exchanging the two coefficient-one squares handles the core-6 ambiguity without a second ternary theorem.  `Algebra/rank_four_weighted_d4_odd_cosets` supplies the `(1,1)` control for every core congruent to 3 modulo 4.  The table-like modules `Algebra/rank_four_weighted_d4_second_unit_covers`, `Algebra/rank_four_weighted_d4_both_unit_covers`, and `Algebra/rank_four_weighted_d4_second_third_double_covers` prove 7, 13, and 6 forms.  The aggregate `Algebra/rank_four_weighted_d4_covers` now exposes all 57 selected weighted-`d=4` theorems; `Matrix.one_two_four_converse_of_three_squares` discharges their common converse input from `Matrix.ThreeSquaresConverse`.  The selected conditional-universality total is 147, leaving 60 of the 207 selected rank-four forms without conditional universality proofs, while all 57 weighted-`d=4` forms add no deep theorem beyond the central three-squares converse.
- `S5.U2` `[~]` (XL) ★★ Apply `S5.U1` across all **207** selected rank-4 escalators. **CONDITIONAL COVERAGE COMPLETE 2026-07-23:** six forms are kernel-certified exact truants and all other 201 have conditional universality proofs. The remaining debt is precisely the named classical converse interfaces, not another rank-four cover. — owes: S4.N1, S4.S*
- `S5.U3` `[x]` (M) Handle the six forms that resolve on their next escalation. `PROOF_FIFTEEN_THEOREM_ASSEMBLY.md`, Sections 3--5, proves that each represents every positive integer except its truant (10 or 15), conditional only on the exact one-two-six/one-two-five converses and finite witness certificates. `Algebra/exceptional_representation` now supplies both the generic rank-raising escalation theorem and the stronger ambient-sublattice theorem: a lattice containing the exceptional form is universal once it represents the one missing value. This also covers same-rank integral overlattices. `Algebra/rank_four_exceptional_odd_c4_r0_c3` proves the first truant-10 parent represents every positive integer except 10 from `Matrix.OneTwoSixConverse`, and packages its escalation theorem. **ALL SIX CERTIFICATES DONE 2026-07-24:** `Algebra/rank_four_exceptional_odd_c4_r2_c7` handles the second truant-10 form through its orthogonal `x²+2y²+5z²` sublattice and norm-330 complement (Section 3.2's argument), and `Algebra/rank_four_exceptional_weighted_d5` handles the four truant-15 forms through the shared 5-adic tail (`Matrix.weightedD5_tail_represents`, extracted from the universal cover) with generated witness prefixes, all conditional only on `Matrix.OneTwoFiveConverse`; the square-scaled truant families descend to fixed 250/375 witnesses. Each form's `*_escalation_universal` corollary discharges every escalation. — owes: S5.U1, S3.E5
- `S5.F1` `[x]` (M) ★★ **Fifteen Theorem:** combine `S3.C2` + `S5.U2` + `S5.U3`. `PROOF_FIFTEEN_THEOREM_ASSEMBLY.md`, Section 7, gives the complete human-readable assembly and its exact dependency ledger. **FORMALIZED 2026-07-24** (`Algebra/fifteen_theorem.math`): both forms are kernel-checked — `Matrix.universal_of_critical_values` (nine critical hypotheses) and `Matrix.fifteen_theorem` (represents 1…15), plus the trivial converse `Matrix.universal_represents_values_through_fifteen`. The rank-four endgame runs through the generated 207-way dispatch `Matrix.selectedRankFourNormalForm_outcome` (`Algebra/rank_four_outcome_dispatch_generated.math`, `scripts/generate_rank_four_outcome_dispatch.py`): every selected normal form is conditionally universal or one of the six exceptional co-singleton forms with its truant certificate; universal sublattices finish by `represents_of_sublattice`, exceptional ones by `universal_of_exceptional_sublattice` with the critical 10/15 hypotheses. **EXACT REMAINING DEBT** of the unconditional theorem, as of 2026-07-25: the seven classical converse interfaces (`ThreeSquaresConverse`, `TripleSquaresConverse`, `OneTwoThreeConverse`, `OneTwoSixConverse`, `Integer.TwoThreeSixLocalConverse`, `OneTwoFiveConverse`, `DetSevenSafeConverse` — one-two-four and one-three-six are derived in the dispatch) — and nothing else. The former second debt, `Matrix.RankThreeEscalatorExclusion`, is discharged: all nine certificates are proved and both `Matrix.universal_of_critical_values` and `Matrix.fifteen_theorem` have lost that hypothesis (see the S3.C2 box). — owes: S3.C2, S5.U2, S5.U3
- `S5.F2` `[~]` (S/M) *(Optional)* Minimality of {1,2,3,5,6,7,10,14,15}. **DESIGN SETTLED 2026-07-25** — the statement splits in two, and the owner chose WEAK first. **WEAK DONE 2026-07-25** (`Algebra/diagonal_forms` + `Algebra/critical_value_minimality`, both manifest-added at ZERO leaks, budget 397 held): `Matrix.critical_values_minimal (v) (critical) : Matrix.CriticalValueIsNeeded(v)` and the non-universality reading `Matrix.critical_values_do_not_force_universality`, both UNCONDITIONAL. The witnesses are exactly the strong candidates, all nine diagonal: (2,2,3,4), (1,3,3,5), (1,1,4,5), (1,2,6,6), (1,1,3,7), (1,1,1,9), (1,2,3,11), (1,1,2,15) and — for 15 — (1,2,5,5). Infrastructure that landed with it, reusable for STRONG: the diagonal ladder `Matrix.diagonalUnaryForm`/`BinaryForm`/`TernaryForm`/`QuaternaryForm` (iterated `diagonalExtension` from `identity(r, 0)`) with symmetry, positive-definiteness and coordinate value law at each rank; the two working laws `Matrix.diagonalExtension_appendCoordinate_value` and `Matrix.diagonalExtension_not_represents_below_corner` (+ the four per-rank rungs); `Matrix.diagonalExtension_identity` and the `<namedForm>_is_diagonal` bridges identifying `squareForm`, `sumOfTwoSquaresForm`, `squarePlusDoubleSquareForm`, `sumOfThreeSquaresForm`, `sumOfTwoSquaresPlusDoubleSquareForm`, `sumOfTwoSquaresPlusTripleSquareForm`, `squarePlusDoubleSquarePlusTripleSquareForm`, `squarePlusDoublePlusScaledSquareForm(d)` with their diagonal spellings; `Matrix.diagonalQuaternaryForm_represents` (a witness tuple + ground arithmetic); `Integer.le_multiply_by_at_least_one`. The `v`=15 witness is connected to the classified `squarePlusDoublePlusScaledRankFourRepresentative(5, 0, 0, 5)` by `diagonalExtension_isometric_borderedAssembly` at the zero vector, so its exclusion is the generated exact-truant certificate transported along the isometry, and its positive-definiteness comes from the ladder (the selected rank-four forms have no PD facts of their own).
  - **WEAK** (no proper subset of the nine is forcing): per critical `v`, a symmetric positive-definite form representing the other eight and missing `v`. Unconditional, and the "misses `v`" half is almost always an existing lemma. **BUILD ORDER DECIDED 2026-07-25: use the STRONG candidates below as the weak witnesses**, so the weak proofs upgrade in place — same forms, and only the "represents" half is restated — instead of being discarded when strong lands. That makes all nine witnesses uniform rank-four diagonal forms, which needs a general diagonal-quaternary constructor and its coordinate value law (new, reusable, and required for strong anyway); the cheapest-per-value witnesses, kept here for reference, would have been mixed rank three and four: `v`=1 → 2x²+2y²+2z²+3w² (minimum ≥ 2); `v`=2 → x²+3y²+3z²+4w²; `v`=3 → x²+y²+5z² (¬R(x²+y², 3)); `v`=5 → x²+2y²+6z² (¬R(x²+2y², 5)); `v`=6 → x²+y²+3z²+7w² (`Integer.two_squares_plus_triple_square_not_six`, proved for the exclusion certificates); `v`=7 → x²+y²+z²+9w² (three squares misses 7); `v`=10 → x²+2y²+3z² (truant 10, existing); `v`=14 → x²+y²+2z² (truant 14, existing); `v`=15 → x²+2y²+5z²+5w². The two truant witnesses are nearly free — everything below the truant comes from `IsTruant.represents_below`, so only the critical values ABOVE `v` need witness vectors (`v`=14 needs 15 alone; `v`=10 needs 14 and 15). Non-universality is `IsTruant.not_universal`, or immediate from missing `v`.
  - **STRONG** (every forcing set contains all nine — the statement Wikipedia records, and what "unique minimal" means): needs co-singleton witnesses, i.e. represents EVERY positive integer except `v`. `v`=10, 15 are done (S5.U3's six exceptional forms). A bounded search (every n ≤ 400) finds a diagonal quaternary for each of the other seven, all of the shape **(escalator with truant `v`) ⊥ (one diagonal coefficient > `v`)**: 1 → (2,2,3,4); 2 → (1,3,3,5); 3 → (1,1,4,5); 5 → (1,2,6,6); 6 → (1,1,3,7); 7 → (1,1,1,9); 14 → (1,1,2,15). The big coefficient makes "misses `v`" collapse onto the parent's truant fact, so the whole difficulty is universality-except-`v` — a residual cover of the ternary's obstructed classes by shifts of the fourth coordinate, i.e. exactly the `three_squares_residual_cover_*` / `generate_det_seven_residual_covers.py` pipeline, over the SAME seven converses. No new deep input; several witnesses per value give slack when a cover is awkward. Unplaced on an existing interface: `v`=1 (no norm-one vector at all) and `v`=3. — owes: S5.F1, S3.C1
