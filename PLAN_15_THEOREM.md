...
- `S3.E4` `[~]` (M) The rank-3 escalators (explicit deduplicated list). — owes: S3.E2, S2.R4 · **WIP 2026-07-19:** the arithmetic
  ...
  Thus every rank-3 escalation now has a known corner and an explicit `escalation_border_bound`; next is to enumerate the two
  bounded border families before isometry deduplication.
