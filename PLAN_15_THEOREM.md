...
- `S0.D2` `[~]` (S) `det I = 1`; `det Aᵀ = det A`. — owes: S0.D1, S0.M3 · **`det Aᵀ = det A` DONE 2026-07-18** (`Algebra/determinant_transpose.math`: entry-product reindex along σ + `sign_inverse` + summing over the inversion rearrangement of the permutation list; new general lemmas `Permutation.inverse_inverse`, `Permutation.sign_inverse`, `inverse_map_permutes_allPermutations`). `det I = 1` remains (needs a product-of-ones and a zero-factor-kills-product aggregation lemma + `sumOver_single` delta-collapse).
