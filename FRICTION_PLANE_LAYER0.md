...
(`PLAN_JORDAN_SCHOENFLIES.md`) — 84 theorems across
...
writes without pausing.** A `linarith` over ordered fields would absorb
I2–I7 outright.
...
### I2 — strict/non-strict asymmetry, in both directions
...
- `-x ≤ c` from `-c ≤ x` needs `Real.LessOrEqual.negate` cited.
  `Real.LessThan.negate` is marked `automatic`; the weak one is not.
  Bit `Real.absolute_value_LessOrEqual_of_bounds`.
- `0 < c - a` from `a < c` is **not** automatic, while `0 ≤ c - a` from
  `a ≤ c` **is**. Bit `Real.maximum_LessThan`. Workaround: bridge
  through `0 = a - a < c - a`.

### I3 — adding two inequalities is not a step
...
This is the **most frequent** friction in the whole batch — it appears in
`norm.math` twice and `maximum.math` twice.
...
### I5 — missing strict counterparts
...
Both bit `Real.maximum_LessThan`, which is **parked, not proved** — see
the tail of `Real/maximum.math`. It is wanted for convexity of balls
(`max(‖u‖,‖w‖) < r` from two strict bounds).

### I6 — two spellings of one fact: `0 ≤ x` vs `Real.IsNonneg(x)`
...
**Still missing:** `Real.maximum_LessThan` (blocked by I5), and the
