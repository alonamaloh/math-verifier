...
- `Universal.centralGenerators`, `Universal.centralPairs`,
  `Universal.centralSelector` and `Universal.CentrallyAbsorbs` — the three- and
  two-block generating sets of blueprint Theorem 6.1 and Definition 6.2, the
  selector that splits them into two variables, and central absorption, in
  [central.math](central.math).
- `Universal.betaSet`, `Universal.doubledRelation`, `Universal.linkedPair` and
  `Universal.linkedInto` — blueprint `β(P)`, the doubled relation `R'`, the
  relation `T` it projects from, and the elements with a link-neighbour in a
  target set, in [doubling.math](doubling.math).
...
- `Universal.center_central` — **blueprint Theorem 6.1**: the diagonal at a
  non-central point is not generated by the three blocks. It needs neither
  finiteness nor a Taylor term. The generators are not sorted: the variable type
  of the realising term *is* the generating set, so the selector is just "is
  this generator's second coordinate the point", and blueprint Lemma 1.20 is
  never needed.
- `Universal.Absorbs.of_star_witness` — a witness over a star power's index type
  gives absorption at the numeric arity `arity ^ depth`. The only place a star
  power's arity is named as a number.
- `Universal.zhuk_center` — **blueprint Corollary 6.3**, Zhuk's centre theorem:
  the left centre of a subdirect relation centrally absorbs.
...
- `Universal.IsEssential.reindex` — blueprint Lemma 1.19(e) for essentiality: a
  bijection of index types carries an essential relation to one over the new
  index. This is what lets the doubled relation be built over a disjoint union
  of two halves and only afterwards presented on an initial segment.
- `Universal.betaSet_subset_generated` — **blueprint Step 1 (‡)**: with the
  relation minimizing `|β|`, every element of `β` is generated by the subset
  together with any other element of `β`. Stated as a standalone universally
  quantified lemma, before the element `b` of Step 2 is fixed — blueprint
  Remark 7.2 — because Step 3 instantiates it twice, the second time at an
  element the first instantiation produced.
- `Universal.linking_disjoint` — **blueprint Step 3**: the linking relation
  misses `β × β`. The only argument in the development that doubles back on
  itself.
- `Universal.HasEssential.doubled` — **blueprint Lemma 7.1**, the Zhuk–Kozik
  essential doubling trick, specialized to the case its only consumer needs:
  a `subset`-essential relation of arity `1 + size` yields one of arity
  `size + size`. Only the carrier's finiteness is used, for the minimization.
- `Universal.centrally_absorbs_ternary` — **blueprint Corollary 8.1**: central
  absorption is witnessed by a ternary term. Doubling the largest essential
  arity exceeds it.
- `Universal.leftCenter_ternary_witness` — **the main theorem**: for a subdirect
  `R ≤ A × B` with `B` Taylor and without a proper binary absorbing subalgebra,
  a ternary term witnesses that the left centre of `R` absorbs `A`.
