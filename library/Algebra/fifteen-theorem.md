...
- The first two rank-four branches are complete. Above `x²+y²+z²`,
  `Matrix.sumOfThreeSquaresRankFourEscalation_classified` places every actual
  escalation in one of seven determinant-separated squared-norm classes. Above
  `x²+y²+3z²`,
  `Matrix.sumOfTwoSquaresPlusTripleSquareRankFourEscalation_classified` derives
  the exact bound `3a²+3b²+c² < 18` and places every actual escalation in one
  of 18 kernel-certified top-shear normal forms. The latter finite classifier is
  generated by `scripts/generate_rank_four_diagonal_branch.py`; its 109 valid
...
`rank_four_diagonal_branch_coverage`.
