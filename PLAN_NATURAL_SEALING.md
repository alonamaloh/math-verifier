...
| 2 | Tactics speak the boundary | **not started** | `case base:`/`case step(k):` already routes through `<Carrier>.induction_on_one_plus` and works at the sealed alias UNCHANGED (probe-validated). What must be built: (a) the modern `case n = 0` / `case n = k + 1` spellings parse to constructor patterns and take the RAW-recursor path (probe: fails at seal with "type 'Natural' is not an inductive", cases.cpp:1167) — retarget them, plus plain `cases n`, onto boundary theorems via an opaque-INDUCTIVE-alias probe (sibling of `engageOpaqueQuotientAlias`, cases.cpp:1101); the case-analysis boundary theorem **`Natural.cases_on_one_plus` now exists** (Stage 1); still needed is a boundary-layout twin of `buildCaseLambda` (or hand-assembly mirroring `elaborateByInductionOnePlus`, induction.cpp:1056-1081); (b) pattern-match definitions at the alias (`\| 0 => \| 1+k =>`, patterns.cpp:9) compile onto `Natural.recursion` (to be built here, its only consumer) incl. structural-recursion rewriting — the hardest piece; (c) the parse-time numeral-pattern desugar (parser.cpp:2725-2748, 4075-4089) emits bare `zero`/`successor` tokens — post-flip these resolve as the wrapper definitions, so the pattern path must keep accepting them as constructor labels (the last-component matcher landed in Stage 1 covers the label side). |
...
**Overall:** Stages 0 and 1 done 2026-07-10. The library builds with
`Natural` as a transparent alias over the qualified raw inductive, all
gates green. Stage 2 (tactics speak the boundary) is the next
executable step; Stage 3 (consumer migration) can proceed in parallel.
...
- **Stage 2 (tactics speak the boundary) is the next executable step.**
  The scratch branch `scratch/natural-seal-stage0-probe` (3c65a529)
  still holds the opaque-endpoint probes (note: it predates the Stage-1
  orientation flip, so its bare-raw-constructor spellings are historic;
  the validated mechanics — unfold-rescue, bridge, base/step routing —
  carry over).
