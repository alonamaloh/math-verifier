...
FORMS = (
    ...
    Form("q1c7odd", "m315_odd", 1, 7, 315, (1, 2, 3, 4, 5, 6), "DetSevenSelectedM315Odd", 63, 120, True),
)
...
def source(form: Form, value: int) -> bool:
    carrier_condition = value % 2 == 1 if form.odd_only else value % 4 != 0
    return carrier_condition and value % 49 != 0 and not safe(value)
...
def render_even_prefix_part(
  ...
  Matrix.represents_by_witness(A, target := {target}, vector := {vector(witness)},
    ...
    return f"""theorem Matrix.detSevenFiniteQ1C7Even_part{lower // QUOTIENT_CHUNK_SIZE}
        : ∀ (q : ℕ). q ≥ {lower} → q < {upper} →
          Matrix.Represents(
...
def render_even_prefix(form: Form) -> str:
    assert form.slug == "q1c7odd"
    witness_table = witnesses(form, 90)
    parts = [
        render_even_prefix_part(form, lower, min(lower + QUOTIENT_CHUNK_SIZE, 46), witness_table)
        for lower in range(0, 46, QUOTIENT_CHUNK_SIZE)
    ]
    dispatch = render_quotient_dispatch(
        Form("q1c7even", "", 1, 7, 0, (), "", 0, 0),
        0,
        0,
        46,
        "  ",
    ).replace("detSevenFiniteQ1C7EVENA0T0", "detSevenFiniteQ1C7Even")
    return f"""-- Generated by scripts/generate_det_seven_finite_covers.py. Do not edit.
-- Direct witnesses for the even prefix 0,2,...,90 of q1c7.

module Algebra.det_seven_finite_q1c7_even_generated

import Algebra.det_seven_rank_four_infrastructure

{chr(10).join(chr(10) + part for part in parts)}

theorem Matrix.detSevenEvenFiniteCover_q1c7
        (n : ℕ) (bounded : n ≤ 90) (even : Natural.modulo(n, 2) = 0)
        : Matrix.Represents(
            Matrix.squarePlusDoubleSquareOddRankFourRepresentative(4, 1, 7),
            (n : ℤ)) := {{
  let q : ℕ := Natural.floor_divide(n, 2);
  2 * q + Natural.modulo(n, 2) = n by Natural.floor_divide_modulo_decompose;
  2 * q + 0 = n by substituting even as exactDecomposition;
  q < 46 by Natural.quotient_below_of_bounded_decomposition(
    2, q, 0, n, 90, 46, done, exactDecomposition, bounded)
  as qBelow;
  Matrix.Represents(
      Matrix.squarePlusDoubleSquareOddRankFourRepresentative(4, 1, 7),
      (2 : ℤ) * (q : ℤ)) by {{
{dispatch}
  ...
  (2 : ℤ) * (q : ℤ)
     = ((2 * q : ℕ) : ℤ) by Equality.symmetry(Natural.to_integer.multiply_preserves(2, q))
     = (n : ℤ) by substituting exactDecomposition;
  done
}}
"""
...
def generated_files(form: Form) -> dict[Path, str]:
    ...
    if form.slug == "q1c7odd":
        result[OUTPUT_DIR / "det_seven_finite_q1c7_even_generated.math"] = render_even_prefix(form)
    return result
